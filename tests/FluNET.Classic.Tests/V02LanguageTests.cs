...
public sealed class V02LanguageTests
{
    [Test]
    public async Task Typed_collection_stages_preserve_element_values()
    {
        ...
        RuntimeResult grouped = await engine.RunAsync("GROUP [users] BY Active INTO [groups].", state); Assert.That(((CollectionGroup[])grouped.Result!).Length, Is.EqualTo(2));
    }
