...
if (project?.Manifest.Execution is { } execution)
{
    ...
    {
        if (execution.Timeout is { } timeout) policy.DefaultTimeout = timeout;
        if (execution.Parallelism is { } parallelism) policy.MaxParallelism = parallelism;
    };
}
...
{
    if (project is not null) return await File.ReadAllTextAsync(project.EntryFile);
...
{
    if (command is not ("run" or "check" or "format" or "plan" or "explain")) return null;
