...
internal enum VerbosityLevel { Normal, Summary, Stages, Trace }
internal enum ColorMode { Auto, Always, Never }
...
internal static class CliArgumentParser
{
    public static bool TryParse(string[] args, out CliOptions options, out string? error)
    {
        ...
        {
            string value = args[i];
            if (value.Equals("--deny-by-default", StringComparison.OrdinalIgnoreCase)) { deny = true; continue; }
            if (value.Equals("--allow", StringComparison.OrdinalIgnoreCase))
            {
                if (++i >= args.Length) { error = "--allow requires a capability name."; break; }
                allowed.Add(args[i]); continue;
            }
            if (value.Equals("-v", StringComparison.OrdinalIgnoreCase) || value.Equals("--verbose", StringComparison.OrdinalIgnoreCase)) { verbosity = verbosity < VerbosityLevel.Summary ? VerbosityLevel.Summary : verbosity; continue; }
            if (value.Equals("-vv", StringComparison.OrdinalIgnoreCase)) { verbosity = verbosity < VerbosityLevel.Stages ? VerbosityLevel.Stages : verbosity; continue; }
            if (value.Equals("-vvv", StringComparison.OrdinalIgnoreCase)) { verbosity = VerbosityLevel.Trace; continue; }
            if (value.Equals("--color", StringComparison.OrdinalIgnoreCase))
            {
                if (++i >= args.Length) { error = "--color requires auto, always, or never."; break; }
                if (!Enum.TryParse(args[i], true, out color)) { error = $"Unknown color mode '{args[i]}'. Use auto, always, or never."; break; }
                continue;
            }
            if (value.StartsWith("-", StringComparison.Ordinal) && value is not "-") { error = $"Unknown option '{value}'."; break; }
            arguments.Add(value);
        }
...
internal sealed class ConsoleExecutionObserver : IExecutionObserver
{
    ...
    public ConsoleExecutionObserver(TextWriter writer, VerbosityLevel verbosity, ColorMode color, bool redirected)
    {
        ...
        _color = color switch { ColorMode.Always => true, ColorMode.Never => false, _ => !redirected && Environment.GetEnvironmentVariable("NO_COLOR") is null };
    }

    public ValueTask OnEventAsync(ExecutionEvent item, CancellationToken cancellationToken = default)
    {
        if (_verbosity == VerbosityLevel.Normal) return ValueTask.CompletedTask;
        if (_verbosity == VerbosityLevel.Summary && item.Kind is not (ExecutionEventKind.RunStarted or ExecutionEventKind.RunCompleted or ExecutionEventKind.Diagnostic)) return ValueTask.CompletedTask;
        if (_verbosity == VerbosityLevel.Stages && item.Kind is ExecutionEventKind.LoopItem) return ValueTask.CompletedTask;
        ...
        lock (_gate) _writer.WriteLine(Paint(line, item));
    ...
    public void WriteDiagnostic(string code, string message, bool warning = false)
    {
        ...
        lock (_gate) _writer.WriteLine(_color ? $"\u001b[{(warning ? "33" : "31")}m{line}\u001b[0m" : line);
    }
}
