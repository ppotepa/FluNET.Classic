...
public static class FluNetHostingExtensions
{
    ...
    {
        options ??= new FluNetOptions(); ILanguageModule[] modules = options.Modules.ToArray();
        services.AddSingleton(options); services.AddSingleton(modules); services.AddSingleton<LanguageCompiler>();
        services.AddSingleton(sp => sp.GetRequiredService<LanguageCompiler>().Build(modules: modules));
        services.AddSingleton(sp => sp.GetRequiredService<LanguageBuildResult>().ThrowIfFailed());
        services.AddSingleton<LanguageIntrospectionService>(); services.AddSingleton<ClassicLanguageService>();
        services.AddSingleton(sp => { var r = new ValueResolverRegistry(); options.ConfigureResolvers?.Invoke(r); return r; });
        services.AddSingleton(sp => { var r = new ValueConversionRegistry(); options.ConfigureConverters?.Invoke(r); return r; });
        services.AddSingleton<ICapabilityPolicy>(_ => options.AllowedCapabilities is null ? new AllowAllCapabilityPolicy() : new CapabilitySetPolicy(options.AllowedCapabilities));
        services.AddSingleton(new HttpClient()); services.AddSingleton<IOutputWriter, ConsoleOutputWriter>(); services.AddSingleton<IEmailSender, MissingEmailSender>();
        services.AddSingleton<ClassicLexer>(); services.AddTransient(sp => new ClassicParser(sp.GetRequiredService<LanguageSnapshot>(), sp.GetRequiredService<ClassicLexer>()));
        services.AddTransient(sp => new SemanticBinder(sp.GetRequiredService<LanguageSnapshot>(), sp.GetRequiredService<ValueResolverRegistry>(), sp.GetRequiredService<ValueConversionRegistry>(), sp));
        services.AddTransient(sp => new BoundExecutor(sp.GetRequiredService<ValueConversionRegistry>(), sp.GetRequiredService<ICapabilityPolicy>(), sp)); services.AddTransient<ClassicEngine>();
        return services;
    }
}
