...
public static class FluNetHostingExtensions
{
    public static IServiceCollection AddFluNetClassic(this IServiceCollection services, FluNetOptions? options = null)
    {
        ...
        services.AddSingleton<LanguageIntrospectionService>();
        services.AddSingleton<ClassicLanguageService>();
        ...
        services.AddTransient(sp => new BoundExecutor(sp.GetRequiredService<ValueConversionRegistry>(), sp.GetRequiredService<ICapabilityPolicy>(), sp));
...
{
    public static ServiceProvider Create(FluNetOptions? options = null, Action<IServiceCollection>? configure = null)
    {
        var services = new ServiceCollection();
        services.AddFluNetClassic(options);
...
{
    public ValueTask SendAsync(string to, string message, CancellationToken cancellationToken = default) =>
        throw new InvalidOperationException("No IEmailSender is registered. Configure one in the host before using SEND.");
}
