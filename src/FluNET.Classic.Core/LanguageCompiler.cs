...
public sealed class LanguageCompiler
{
    ...
    private VerbImplementationDescriptor? CompileVerb(Type type, ICollection<LanguageDiagnostic> diagnostics)
    {
        ...
        SentencePattern[] patterns = constructors.Select((constructor, index) => new SentencePattern($"pattern:{Slug(name)}:{Slug(type.Name)}:{index}", constructor, constructor.Parameters.Where(x => !x.IsService && x.RoleName is not null).Select(x => new RoleSlotDescriptor($"role:{Slug(name)}:{index}:{x.Position}", x.RoleName!, x.ParameterType, x.TypeShape, x.Direction, x.Cardinality, x.Position, x.Name, !x.IsOptional)).ToArray())).Where(x => x.Roles.Count > 0).ToArray();
    ...
    private ParameterDescriptor CompileParameter(ParameterInfo parameter, string verbName)
    {
        ...
        return new(parameter, parameter.Name ?? $"arg{parameter.Position}", parameter.ParameterType, ClrTypeShape.From(parameter.ParameterType, nullability.ReadState), parameter.IsOptional, parameter.HasDefaultValue ? parameter.DefaultValue : null, paramArray, service, roleName, direction, cardinality, parameter.Position);
    }
    ...
    private static IEnumerable<string> InferCapabilities(Type type, IEnumerable<SentencePattern> patterns)
    {
        ...
        if ((typeof(IGet).IsAssignableFrom(type) || typeof(ILoad).IsAssignableFrom(type)) && fileInput) yield return StandardCapabilities.FileSystemRead;
        if ((typeof(ISave).IsAssignableFrom(type) || typeof(IDelete).IsAssignableFrom(type) || typeof(IDownload).IsAssignableFrom(type)) && fileInput) yield return StandardCapabilities.FileSystemWrite;
    ...
    {
        if (typeof(ITransform).IsAssignableFrom(type)) yield return ExecutionTrait.Pure;
        if (typeof(IGet).IsAssignableFrom(type) || typeof(ILoad).IsAssignableFrom(type)) yield return ExecutionTrait.Idempotent;
        if (typeof(ISave).IsAssignableFrom(type) || typeof(IDelete).IsAssignableFrom(type) || typeof(IPost).IsAssignableFrom(type) || typeof(ISend).IsAssignableFrom(type) || typeof(IDownload).IsAssignableFrom(type)) yield return ExecutionTrait.SideEffecting;
    ...
    private static string? ResolveVerbName(Type type)
    {
        ...
        (Type Marker, string Name)[] families = { (typeof(IGet), "GET"), (typeof(ISave), "SAVE"), (typeof(ILoad), "LOAD"), (typeof(ISend), "SEND"), (typeof(IDownload), "DOWNLOAD"), (typeof(IDelete), "DELETE"), (typeof(IPost), "POST"), (typeof(ITransform), "TRANSFORM"), (typeof(ISay), "SAY") };
    ...
    private static string? InferRoleName(string? name) => name?.ToLowerInvariant() switch { "what" or "value" or "message" or "body" or "content" => "WHAT", "from" or "source" => "FROM", "to" or "target" or "destination" => "TO", "using" or "format" or "strategy" => "USING", "with" or "options" => "WITH", "then" => "THEN", _ => null };
    private static RoleDirection InferDirection(string verb, string? role) => role?.Equals("WHAT", StringComparison.OrdinalIgnoreCase) == true && (verb is "GET" or "LOAD" or "DOWNLOAD") ? RoleDirection.Output : RoleDirection.Input;
