...
public abstract class ValueConverter<TSource, TTarget> : IValueConverter<TSource, TTarget>
{
    ...
    bool IValueConverter.TryConvert(object? value, out object? result) { if (value is TSource source && TryConvert(source, out TTarget? converted)) { result = converted; return true; } result = null; return false; }
}
...
public sealed class ValueConversionRegistry
{
    ...
    {
        Type t = Nullable.GetUnderlyingType(target) ?? target; Type s = Nullable.GetUnderlyingType(source) ?? source;
        if (s == t) { kind = ConversionKind.Exact; cost = 0; return true; }
        if (target.IsAssignableFrom(source) || t.IsAssignableFrom(s)) { kind = ConversionKind.Assignable; cost = 1; return true; }
        if (_converters.ContainsKey((source, target)) || _converters.ContainsKey((s, t))) { kind = ConversionKind.Registered; cost = 2; return true; }
        if (IsNumeric(s) && IsNumeric(t)) { kind = ConversionKind.Numeric; cost = 3; return true; }
        kind = default; cost = int.MaxValue; return false;
    }
    public bool TryConvert(object? source, Type target, out ConversionResult? result)
    {
        if (source is null) { bool nullable = !target.IsValueType || Nullable.GetUnderlyingType(target) is not null; result = nullable ? new(null, ConversionKind.Assignable, 1) : null; return nullable; }
        Type sourceType = source.GetType(); if (!CanConvert(sourceType, target, out ConversionKind kind, out int cost)) { result = null; return false; }
        ...
        if (kind == ConversionKind.Registered) { IValueConverter converter = _converters.TryGetValue((sourceType, target), out IValueConverter? exact) ? exact : _converters[(sourceType, targetType)]; if (converter.TryConvert(source, out object? converted)) { result = new(converted, kind, cost); return true; } }
        if (kind == ConversionKind.Numeric) { try { result = new(Convert.ChangeType(source, targetType, CultureInfo.InvariantCulture), kind, cost); return true; } catch { } }
