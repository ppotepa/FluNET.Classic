...
public sealed class SemanticBinder
{
    ...
    public BoundScript Bind(ScriptNode script, IReadOnlyDictionary<string, Type>? initialVariables = null)
    {
        ...
        if (initialVariables is not null)
            foreach ((string name, Type type) in initialVariables) symbols.Define(name, type);
    ...
    private BoundStatement? BindStatement(StatementNode statement, SymbolScope symbols) => statement switch
    {
        PipelineNode pipeline => BindPipeline(pipeline, symbols),
        IfNode conditional => BindIf(conditional, symbols),
        ForEachNode loop => BindForEach(loop, symbols),
        _ => null
    };
    ...
    private CandidateResult TryCandidate(SentenceNode sentence, VerbDescriptor verb, VerbImplementationDescriptor implementation, SentencePattern pattern, SymbolScope symbols, Type? pipelineType, QualifierDescriptor? qualifier)
    {
        ...
        foreach (ClauseNode clause in sentence.Clauses)
        {
            ...
            if (!supplied.TryGetValue(matches[0].Name, out Queue<ExpressionNode>? values)) supplied[matches[0].Name] = values = new Queue<ExpressionNode>();
    ...
    {
        BoundValue? source = BindCollectionSource(node.Source, symbols, pipelineType, node.Span, node.Operation, "FLU-BIND-160");
        if (source is null) return null;
        Type? element = ClrTypeShape.GetElementType(source.Type);
        if (element is null)
        {
            _diagnostics.Add(new("FLU-BIND-161", $"{node.Operation} requires a collection, got {source.Type.Name}.", node.Span));
            return null;
        }
    ...
    private BoundExpression BindUnary(UnaryExpression unary, SymbolScope symbols, Type? itemType)
    {
        ...
        if (!_language.TryGetOperator(unary.Operator, out OperatorDescriptor descriptor) || descriptor.Arity != OperatorArity.Unary)
        {
            ...
            return new BoundUnaryExpression(unary.Operator, operand, typeof(bool), unary.Span);
        }
        if (descriptor.Semantic == OperatorSemanticKind.Logical && operand.Type != typeof(bool))
            _diagnostics.Add(new("FLU-BIND-155", $"Operator '{descriptor.Name}' requires BOOLEAN, got {Friendly(operand.Type)}.", unary.Span));
        return new BoundUnaryExpression(descriptor.Name, operand, typeof(bool), unary.Span);
    }
    ...
    {
        if (expression is ReferenceExpression && descriptor.ReferenceOperandType is { } referenceType &&
            TryBindValue(expression, referenceType, RoleDirection.Input, "EXPRESSION", null, symbols, out BoundValue? resolved, out _))
    ...
    private BoundExpression BindBinary(BinaryExpression binary, SymbolScope symbols, Type? itemType)
    {
        ...
        ValidateBinaryOperator(descriptor, left, right, binary.Span);
        return new BoundBinaryExpression(left, descriptor, right, typeof(bool), binary.Span);
    }
    ...
    private BoundExpression BindBetween(BetweenExpression between, SymbolScope symbols, Type? itemType)
    {
        ...
        if (!AreOrderCompatible(operand.Type, lower.Type) || !AreOrderCompatible(operand.Type, upper.Type))
            _diagnostics.Add(new("FLU-BIND-157", $"BETWEEN requires comparable bounds; got {Friendly(operand.Type)}, {Friendly(lower.Type)}, {Friendly(upper.Type)}.", between.Span));
        return new BoundBetweenExpression(operand, lower, upper, between.Span);
    }
    ...
    {
        bool valid = descriptor.Semantic switch
        {
            OperatorSemanticKind.Custom => true,
            OperatorSemanticKind.Logical => left.Type == typeof(bool) && right.Type == typeof(bool),
            OperatorSemanticKind.Equality => AreComparable(left.Type, right.Type),
            OperatorSemanticKind.Ordering => AreOrderCompatible(left.Type, right.Type),
            OperatorSemanticKind.Contains => CanContain(left.Type, right.Type),
            OperatorSemanticKind.StartsWith or OperatorSemanticKind.EndsWith or OperatorSemanticKind.RegexMatch => left.Type == typeof(string) && right.Type == typeof(string),
            OperatorSemanticKind.Membership => CanContain(right.Type, left.Type) && right.Type != typeof(string),
            OperatorSemanticKind.Temporal => IsTemporal(left.Type) && IsTemporal(right.Type) && AreComparable(left.Type, right.Type),
            OperatorSemanticKind.Between => false,
            _ => false
        };
        if (!valid)
            _diagnostics.Add(new("FLU-BIND-158", $"Operator '{descriptor.Name}' is not valid for {Friendly(left.Type)} and {Friendly(right.Type)}.", span));
    }
    ...
    private bool CanContain(Type containerType, Type valueType)
    {
        ...
        if (container == typeof(string)) return value == typeof(string);
        if (typeof(IDictionary).IsAssignableFrom(container)) return true;
    ...
    private bool AreComparable(Type left, Type right)
    {
        ...
        if (a == b || a.IsAssignableFrom(b) || b.IsAssignableFrom(a)) return true;
    ...
    {
        if (!AreComparable(left, right)) return false;
        Type a = Nullable.GetUnderlyingType(left) ?? left;
        Type b = Nullable.GetUnderlyingType(right) ?? right;
    ...
    {
        var context = new ResolutionContext(expected, roleName, verb, qualifier, _services);
