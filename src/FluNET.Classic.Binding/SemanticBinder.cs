...
public sealed class SemanticBinder
{
    ...
    {
        _diagnostics.Clear(); var symbols = new SymbolScope(null); if (initialVariables is not null) foreach ((string n, Type t) in initialVariables) symbols.Define(n, t);
    ...
    private BoundStatement? BindStatement(StatementNode s, SymbolScope symbols) => s switch { PipelineNode p => BindPipeline(p, symbols), IfNode i => BindIf(i, symbols), ForEachNode f => BindForEach(f, symbols), _ => null };
    ...
    private BoundPipeline BindPipeline(PipelineNode p, SymbolScope symbols)
    {
        ...
        {
            BoundStage? bound = stage switch { SentenceNode s => BindSentence(s, symbols, pipelineType), FilterStageNode f => BindFilter(f, symbols, pipelineType), CheckStageNode c => BindCheck(c, symbols), CollectionStageNode c => BindCollection(c, symbols, pipelineType), _ => null };
            ...
            if (bound is BoundCollection { ResultAlias: { Length: > 0 } collectionAlias }) symbols.Define(collectionAlias, bound.ResultType);
        }
    ...
    private CandidateResult TryCandidate(SentenceNode sentence, VerbDescriptor verb, VerbImplementationDescriptor implementation, SentencePattern pattern, SymbolScope symbols, Type? pipelineType, QualifierDescriptor? qualifier)
    {
        ...
        foreach (ClauseNode clause in sentence.Clauses)
        {
            ...
            if (matches.Length == 0) return CandidateResult.Fail($"role {clause.RoleName} is not valid for this pattern"); if (matches.Length > 1) return CandidateResult.Fail($"role {clause.RoleName} is ambiguous in this pattern");
    ...
    {
        BoundValue? source = BindCollectionSource(filter.Source, symbols, pipelineType, filter.Span, "FILTER", "FLU-BIND-120"); if (source is null) return null;
    ...
    private BoundCollection? BindCollection(CollectionStageNode node, SymbolScope symbols, Type? pipelineType)
    {
        ...
        BoundExpression? argument = null; string op = node.Operation.ToUpperInvariant();
        ...
        return new(op, source, element, argument, node.ResultAlias, resultType, node.Span);
    }
    ...
    private BoundValue? BindCollectionSource(ExpressionNode? sourceExpression, SymbolScope symbols, Type? pipelineType, TextSpan span, string operation, string code)
    {
        ...
        {
            if (pipelineType is null) { _diagnostics.Add(new(code, $"{operation} requires a source or pipeline value.", span)); return null; }
    ...
    {
        FluNET.Classic.Syntax.BinaryExpression binary => BindBinary(binary, symbols, itemType), FluNET.Classic.Syntax.UnaryExpression unary => new BoundUnaryExpression(unary.Operator, BindExpression(unary.Operand, symbols, itemType), typeof(bool), unary.Span), PredicateExpression predicate => BindPredicate(predicate, symbols, itemType), IdentifierExpression identifier when itemType is not null => BindItemProperty(identifier, itemType),
    ...
    private BoundExpression BindPredicate(PredicateExpression predicate, SymbolScope symbols, Type? itemType)
    {
        ...
        if (predicate.Predicate.Equals("EXISTS", StringComparison.OrdinalIgnoreCase) && predicate.Operand is ReferenceExpression reference && TryBindValue(reference, typeof(FileInfo), RoleDirection.Input, "EXPRESSION", null, symbols, out BoundValue? file, out _)) operand = new BoundValueExpression(file!, reference.Span); else operand = BindExpression(predicate.Operand, symbols, itemType);
        if (!_predicates.CanEvaluate(predicate.Predicate, operand.Type)) _diagnostics.Add(new("FLU-BIND-152", $"Predicate '{predicate.Predicate}' cannot evaluate {operand.Type.Name}.", predicate.Span)); return new BoundPredicateExpression(predicate.Predicate, operand, predicate.Span);
    }
    ...
    private BoundExpression BindBinary(FluNET.Classic.Syntax.BinaryExpression binary, SymbolScope symbols, Type? itemType)
    {
        ...
        Type result = op is "AND" or "OR" or "=" or "==" or "!=" or ">" or "<" or ">=" or "<=" or "IS" or "IS NOT" or "CONTAINS" or "STARTS WITH" or "ENDS WITH" or "MATCHES" or "IN" or "BEFORE" or "AFTER" ? typeof(bool) : left.Type;
    ...
    private bool TryBindValue(ExpressionNode expression, Type? expected, RoleDirection direction, string verb, string? qualifier, SymbolScope symbols, out BoundValue? bound, out int cost, string? roleName = null)
    {
        ...
        switch (expression)
        {
            ...
            case ReferenceExpression reference:
                ...
                return ResolveText(reference.Value, expected, reference.Span, verb, qualifier, roleName, out bound, out cost);
            case IdentifierExpression identifier:
                if (expected is null) { bound = new BoundConstantValue(identifier.Name, typeof(string), identifier.Span); cost = 0; return true; }
                return ResolveText(identifier.Name, expected, identifier.Span, verb, qualifier, roleName, out bound, out cost);
            default: bound = null; cost = 0; return false;
        }
    }

    private bool ResolveText(string text, Type expected, TextSpan span, string verb, string? qualifier, string? roleName, out BoundValue? bound, out int cost)
    {
        var context = new ResolutionContext(expected, roleName, verb, qualifier, _services); if (_resolvers.TryResolve(text, expected, context, out object? resolved)) { bound = new BoundConstantValue(resolved, expected, span, ConversionKind.Resolution, 4); cost = 4; return true; }
