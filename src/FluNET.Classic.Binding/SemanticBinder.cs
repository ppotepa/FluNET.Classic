...
public sealed class SemanticBinder
{
    ...
    public SemanticBinder(LanguageSnapshot language, ValueResolverRegistry resolvers, ValueConversionRegistry conversions, IServiceProvider? services = null) { _language = language; _resolvers = resolvers; _conversions = conversions; _services = services; }
    ...
    public BoundScript Bind(ScriptNode script, IReadOnlyDictionary<string, Type>? initialVariables = null)
    {
        ...
        if (initialVariables is not null) foreach ((string name, Type type) in initialVariables) symbols.Define(name, type);
    ...
    private BoundPipeline BindPipeline(PipelineNode pipeline, SymbolScope symbols)
    {
        ...
        {
            BoundStage? bound = stage switch { SentenceNode sentence => BindSentence(sentence, symbols, pipelineType), FilterStageNode filter => BindFilter(filter, symbols, pipelineType), _ => null };
            if (bound is null) continue;
            stages.Add(bound); pipelineType = bound.ResultType;
            if (bound is BoundSentence sentence) RegisterOutputs(sentence, symbols);
            if (bound is BoundFilter { ResultAlias: { Length: > 0 } alias }) symbols.Define(alias, bound.ResultType);
        }
    ...
    private BoundSentence? BindSentence(SentenceNode sentence, SymbolScope symbols, Type? pipelineType)
    {
        ...
        if (sentence.Qualifier is not null && !_language.TryGetQualifier(sentence.Qualifier, out qualifier!)) { _diagnostics.Add(new("FLU-BIND-002", $"Unknown qualifier '{sentence.Qualifier}'.", sentence.Span)); return null; }
        ...
        foreach (SentencePattern pattern in implementation.Patterns)
        {
            ...
            if (attempt.Candidate is not null) candidates.Add(attempt.Candidate); else rejected.Add($"{Signature(implementation, pattern)}: {attempt.Reason}");
        }
    ...
    {
        Dictionary<string, Queue<ExpressionNode>> supplied = sentence.Clauses.GroupBy(x => x.RoleName, StringComparer.OrdinalIgnoreCase).ToDictionary(x => x.Key, x => new Queue<ExpressionNode>(x.SelectMany(c => c.Values)), StringComparer.OrdinalIgnoreCase);
        var roles = new List<BoundRole>(); int cost = 0; bool aliasUsedAsOutput = false; bool pipelineUsed = false;
        foreach (RoleSlotDescriptor slot in pattern.Roles.OrderBy(x => x.Position))
        {
            supplied.TryGetValue(slot.Name, out Queue<ExpressionNode>? queue); queue ??= new Queue<ExpressionNode>(); var expressions = new List<ExpressionNode>();
            if (slot.Cardinality is RoleCardinality.ZeroOrMore or RoleCardinality.OneOrMore) while (queue.Count > 0) expressions.Add(queue.Dequeue()); else if (queue.Count > 0) expressions.Add(queue.Dequeue());
            ...
            if (expressions.Count == 0 && slot.Direction == RoleDirection.Input && pipelineType is not null && slot.Required && !pipelineUsed)
            {
                if (TryPlanConversion(pipelineType, SlotBindingType(slot), out ConversionKind kind, out int conversionCost))
                {
                    BoundValue pipeline = new BoundPipelineValue(pipelineType, sentence.Span); if (kind != ConversionKind.Exact) pipeline = new BoundConversionValue(pipeline, SlotBindingType(slot), kind, conversionCost, sentence.Span);
                    roles.Add(new(slot, new[] { pipeline }, sentence.Span)); cost += conversionCost + 1; pipelineUsed = true; continue;
                }
            }
            if (expressions.Count == 0) { if (slot.Required && slot.Cardinality is RoleCardinality.One or RoleCardinality.OneOrMore) return CandidateResult.Fail($"missing {slot.Name}"); continue; }
            var values = new List<BoundValue>(); Type expected = SlotBindingType(slot);
            foreach (ExpressionNode expression in expressions)
            {
                if (!TryBindValue(expression, expected, slot.Direction, verb.Name, qualifier?.Name, symbols, out BoundValue? value, out int valueCost)) return CandidateResult.Fail($"cannot bind {slot.Name} value to {expected.Name}");
    ...
    {
        BoundValue? source;
        if (filter.Source is null) { if (pipelineType is null) { _diagnostics.Add(new("FLU-BIND-120", "FILTER requires a source or pipeline value.", filter.Span)); return null; } source = new BoundPipelineValue(pipelineType, filter.Span); }
        else if (!TryBindValue(filter.Source, null, RoleDirection.Input, "FILTER", null, symbols, out source, out _)) { _diagnostics.Add(new("FLU-BIND-121", "FILTER source cannot be bound.", filter.Source.Span)); return null; }
        Type? element = ClrTypeShape.GetElementType(source!.Type); if (element is null) { _diagnostics.Add(new("FLU-BIND-122", $"FILTER requires a collection, got {source.Type.Name}.", filter.Source?.Span ?? filter.Span)); return null; }
        BoundExpression predicate = BindExpression(filter.Predicate, symbols, element); if (predicate.Type != typeof(bool)) _diagnostics.Add(new("FLU-BIND-123", "WHERE expression must be BOOLEAN.", filter.Predicate.Span));
        return new(source, predicate, element, filter.ResultAlias, filter.Span);
    }

    private BoundIf BindIf(IfNode node, SymbolScope symbols)
    {
        BoundExpression condition = BindExpression(node.Condition, symbols, null); if (condition.Type != typeof(bool)) _diagnostics.Add(new("FLU-BIND-130", "IF condition must be BOOLEAN.", node.Condition.Span));
    ...
    private BoundForEach? BindForEach(ForEachNode node, SymbolScope symbols)
    {
        ...
        var child = new SymbolScope(symbols); child.Define(node.Variable, element); return new(node.Variable, source, element, BindBlock(node.Body, child), node.Span);
    }
    ...
    {
        BoundExpression left = BindExpression(binary.Left, symbols, itemType); BoundExpression right = BindExpression(binary.Right, symbols, itemType); string op = binary.Operator.ToUpperInvariant();
    ...
    {
        if (!TryCompileAccessor(itemType, identifier.Name, out Type? propertyType, out Func<object, object?>? accessor)) { _diagnostics.Add(new("FLU-BIND-150", $"'{itemType.Name}' has no property '{identifier.Name}'.", identifier.Span)); return new BoundValueExpression(new BoundConstantValue(null, typeof(object), identifier.Span), identifier.Span); }
    ...
    private bool TryBindValue(ExpressionNode expression, Type? expected, RoleDirection direction, string verb, string? qualifier, SymbolScope symbols, out BoundValue? bound, out int cost)
    {
        ...
        {
            if (expression is VariableExpression output) { bound = new BoundVariableValue(output.Name, expected ?? typeof(object), true, output.Span); cost = 0; return true; }
        ...
        switch (expression)
        {
            case VariableExpression variable:
                ...
                bound = new BoundVariableValue(variable.Name, variableType, false, variable.Span); return ApplyExpected(ref bound, expected, out cost);
            case PropertyExpression property:
                ...
                bound = propertyValue; return ApplyExpected(ref bound, expected, out cost);
            case InterpolatedStringExpression interpolation:
                ...
                bound = new BoundInterpolatedValue(parts, interpolation.Span); return ApplyExpected(ref bound, expected, out cost);
    ...
    {
        var context = new ResolutionContext(expected, null, verb, qualifier, _services); if (_resolvers.TryResolve(text, expected, context, out object? resolved)) { bound = new BoundConstantValue(resolved, expected, span, ConversionKind.Resolution, 4); cost = 4; return true; }
        bound = null; cost = 0; return false;
    }
    ...
    private bool ApplyExpected(ref BoundValue? value, Type? expected, out int cost)
    {
        ...
        if (kind != ConversionKind.Exact) value = new BoundConversionValue(value, expected, kind, cost, value.Span); return true;
    }
    ...
    {
        if (!TryRoot(property.Target, symbols, out BoundValue? target)) { bound = null; return false; }
        if (!TryCompileAccessor(target!.Type, property.Property, out Type? type, out Func<object, object?>? accessor)) { bound = null; return false; }
        bound = new(target, property.Property, type!, accessor!, property.Span); return true;
    }

    private static bool TryRoot(ExpressionNode expression, SymbolScope symbols, out BoundValue? value)
    {
        if (expression is VariableExpression variable && symbols.TryGet(variable.Name, out Type type)) { value = new BoundVariableValue(variable.Name, type, false, variable.Span); return true; }
        if (expression is PropertyExpression property && TryRoot(property.Target, symbols, out BoundValue? nested) && TryCompileAccessor(nested!.Type, property.Property, out Type? propertyType, out Func<object, object?>? accessor)) { value = new BoundPropertyValue(nested, property.Property, propertyType!, accessor!, property.Span); return true; }
    ...
    private static bool TryCompileAccessor(Type type, string propertyName, out Type? propertyType, out Func<object, object?>? accessor)
    {
        ...
        accessor = System.Linq.Expressions.Expression.Lambda<Func<object, object?>>(System.Linq.Expressions.Expression.Convert(read, typeof(object)), instance).Compile(); propertyType = property.PropertyType; return true;
    }
