...
public enum WorkingDirectoryTarget { CWD }
...
[Verb("SAVE"), Qualifier("CWD"), RequiresCapability(StandardCapabilities.SystemRead), ExecutionTrait(ExecutionTrait.SideEffecting)]
public sealed class SaveWorkingDirectory : IVerb<WorkingDirectory>, ISave, IWhat<DirectoryInfo>, ITo<WorkingDirectoryTarget>, IPipelineConsumer<DirectoryInfo>, IPipelineProducer<WorkingDirectory>
{
    ...
    public SaveWorkingDirectory([What] DirectoryInfo directory, [To] WorkingDirectoryTarget target) { _directory = directory; _target = target; }
    public ValueTask<WorkingDirectory> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (_target != WorkingDirectoryTarget.CWD) throw new NotSupportedException(_target.ToString());
        if (!_directory.Exists) throw new DirectoryNotFoundException(_directory.FullName);
