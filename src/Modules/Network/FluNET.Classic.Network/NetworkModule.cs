...
public sealed record DnsName(string Value) { public override string ToString() => Value; }
public sealed record NetworkPort(int Value) { public NetworkPort(string value) : this(int.Parse(value, System.Globalization.CultureInfo.InvariantCulture)) { } }
public sealed record NetworkEndpoint(DnsName Host, NetworkPort Port);
public sealed record ConnectivityResult(NetworkEndpoint Endpoint, bool Connected, TimeSpan Duration) : IOkState { public bool IsOk => Connected; }
...
[Verb("GET"), Qualifier("ADDRESSES"), RequiresCapability(StandardCapabilities.NetworkDns), ExecutionTrait(ExecutionTrait.Retryable)]
...
[Verb("GET"), Qualifier("CONNECTIVITY"), RequiresCapability(StandardCapabilities.NetworkConnect), ExecutionTrait(ExecutionTrait.Retryable)]
public sealed class GetConnectivity : IVerb<ConnectivityResult>, IGet, IFrom<NetworkEndpoint>, IPipelineProducer<ConnectivityResult>
{
    ...
    {
        var started = System.Diagnostics.Stopwatch.StartNew(); using var client = new TcpClient(); bool connected;
        try { await client.ConnectAsync(_endpoint.Host.Value, _endpoint.Port.Value, cancellationToken).ConfigureAwait(false); connected = true; } catch (SocketException) { connected = false; }
        started.Stop(); return new(_endpoint, connected, started.Elapsed);
    }
}
