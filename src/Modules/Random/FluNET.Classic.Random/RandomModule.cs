...
[Verb("GET")]
[Qualifier("NUMBER")]
[ExecutionTrait(ExecutionTrait.NonDeterministic)]
public sealed class GetRandomNumber : IVerb<decimal>, IGet, IFrom<RandomSource>, IWith<NumberRange>, IPipelineProducer<decimal>
{
    ...
    public GetRandomNumber([From] RandomSource source, [With] NumberRange? range = null)
    {
        _source = source;
        _range = range;
    }
    ...
    {
        int min = _range?.Minimum ?? 0, max = _range?.Maximum ?? int.MaxValue;
        int value = _source == RandomSource.SECURE ? RandomNumberGenerator.GetInt32(min, max) : System.Random.Shared.Next(min, max);
        return ValueTask.FromResult((decimal)value);
    }
}
...
{
    private readonly RandomSource _source; private readonly int _length; public GetRandomBytes([From] RandomSource source, [With] int length)
    {
        _source = source;
        _length = length;
    }
    public ValueTask<byte[]> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default)
    {
        byte[] data = new byte[_length];
        if (_source == RandomSource.SECURE)
