...
public sealed record StorageKey(string Value) { public override string ToString() => Value; }
public sealed record StorageContainer(string Value) { public override string ToString() => Value; }
...
public sealed record StorageObject(StorageKey Key, StorageMetadata Metadata) : IExistenceState { public bool Exists => true; }
...
[Verb("GET"), Qualifier("BINARY"), RequiresCapability(StandardCapabilities.StorageRead), ExecutionTrait(ExecutionTrait.Idempotent), ExecutionTrait(ExecutionTrait.Retryable)]
public sealed class GetStorageBinary : IVerb<byte[]>, IGet, IFrom<StorageKey>, IPipelineProducer<byte[]>
{
    ...
    public GetStorageBinary([From] StorageKey key, [FromServices] IStorageProvider provider) { _key = key; _provider = provider; }
...
[Verb("SAVE"), Qualifier("BINARY"), RequiresCapability(StandardCapabilities.StorageWrite), ExecutionTrait(ExecutionTrait.SideEffecting)]
public sealed class SaveStorageBinary : IVerb<byte[]>, ISave, IWhat<byte[]>, ITo<StorageKey>, IPipelineConsumer<byte[]>, IPipelineProducer<byte[]>
{
    ...
    public SaveStorageBinary([What] byte[] data, [To] StorageKey key, [FromServices] IStorageProvider provider) { _data = data; _key = key; _provider = provider; }
    public async ValueTask<byte[]> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default) { await _provider.SaveAsync(_key, _data, cancellationToken).ConfigureAwait(false); return _data; }
}

[Verb("DELETE"), Qualifier("OBJECT"), RequiresCapability(StandardCapabilities.StorageWrite), ExecutionTrait(ExecutionTrait.SideEffecting)]
...
[Verb("LIST"), Qualifier("OBJECTS"), RequiresCapability(StandardCapabilities.StorageRead), ExecutionTrait(ExecutionTrait.Idempotent)]
...
[Verb("COPY"), Qualifier("OBJECT"), RequiresCapability(StandardCapabilities.StorageRead), RequiresCapability(StandardCapabilities.StorageWrite), ExecutionTrait(ExecutionTrait.SideEffecting)]
public sealed class CopyStorageObject : IVerb<StorageKey>, ICopy, IWhat<StorageKey>, ITo<StorageKey>, IPipelineConsumer<StorageKey>, IPipelineProducer<StorageKey>
{
    ...
    public CopyStorageObject([What] StorageKey source, [To] StorageKey destination, [FromServices] IStorageProvider provider) { _source = source; _destination = destination; _provider = provider; }
    public async ValueTask<StorageKey> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default) { await _provider.CopyAsync(_source, _destination, cancellationToken).ConfigureAwait(false); return _destination; }
}

[Verb("MOVE"), Qualifier("OBJECT"), RequiresCapability(StandardCapabilities.StorageRead), RequiresCapability(StandardCapabilities.StorageWrite), ExecutionTrait(ExecutionTrait.SideEffecting)]
...
{
    private readonly StorageKey _source; private readonly StorageKey _destination; private readonly IStorageProvider _provider;
    public MoveStorageObject([What] StorageKey source, [To] StorageKey destination, [FromServices] IStorageProvider provider) { _source = source; _destination = destination; _provider = provider; }
    public async ValueTask<StorageKey> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default) { await _provider.MoveAsync(_source, _destination, cancellationToken).ConfigureAwait(false); return _destination; }
}
