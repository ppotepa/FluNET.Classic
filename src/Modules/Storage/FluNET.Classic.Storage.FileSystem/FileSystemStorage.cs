...
public sealed class FileSystemStorageProvider : IStorageProvider
{
    ...
    public FileSystemStorageProvider(DirectoryInfo root)
    {
        root.Create();
        _root = Path.GetFullPath(root.FullName);
    }
    ...
    {
        string directory = ResolveContainer(container);
        if (!Directory.Exists(directory))
            return ValueTask.FromResult<IReadOnlyList<StorageObject>>(Array.Empty<StorageObject>());
    ...
    public async ValueTask CopyAsync(StorageKey source, StorageKey destination, CancellationToken cancellationToken = default)
    {
        byte[] data = await GetAsync(source, cancellationToken).ConfigureAwait(false) ?? throw new FileNotFoundException(source.Value);
        await SaveAsync(destination, data, cancellationToken).ConfigureAwait(false);
    }
    ...
    private string ResolvePath(string relative)
    {
        ...
        if (!path.Equals(_root, StringComparison.OrdinalIgnoreCase) && !path.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
