...
public sealed class CsvModule : LanguageModule { public override string Name => "csv"; public override IReadOnlyCollection<QualifierDescriptor> Qualifiers => new[] { new QualifierDescriptor("qualifier:csv-document", "CSV", typeof(CsvDocument)) }; }

[Verb("PARSE"), Qualifier("CSV"), ExecutionTrait(ExecutionTrait.Pure)]
public sealed class ParseCsv : IVerb<CsvDocument>, IParse, IFrom<string>, IWith<CsvOptions>, IPipelineConsumer<string>, IPipelineProducer<CsvDocument>
{
    ...
    public ParseCsv([From] string text, [With] CsvOptions? options = null) { _text = text; _options = options ?? new(); }
    public ValueTask<CsvDocument> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (lines.Length == 0) return ValueTask.FromResult(new CsvDocument(Array.Empty<string>(), Array.Empty<CsvRow>()));
    ...
    private static IEnumerable<string> ParseLine(string line, char delimiter) { var value = new StringBuilder(); bool quote = false; for (int i = 0; i < line.Length; i++) { char c = line[i]; if (c == '"') { if (quote && i + 1 < line.Length && line[i + 1] == '"') { value.Append('"'); i++; } else quote = !quote; } else if (c == delimiter && !quote) { yield return value.ToString(); value.Clear(); } else value.Append(c); } yield return value.ToString(); }
}

[Verb("FORMAT"), Qualifier("CSV"), ExecutionTrait(ExecutionTrait.Pure)]
public sealed class FormatCsv : IVerb<string>, IFormat, IWhat<CsvDocument>, IWith<CsvOptions>, IPipelineConsumer<CsvDocument>, IPipelineProducer<string>
{
    ...
    public FormatCsv([What] CsvDocument document, [With] CsvOptions? options = null) { _document = document; _options = options ?? new(); }
    public ValueTask<string> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default)
    {
        ...
        var lines = new List<string>(); if (_options.HasHeader) lines.Add(string.Join(_options.Delimiter, _document.Headers.Select(Escape)));
