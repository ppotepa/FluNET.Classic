...
public sealed class ListFilesScoped : IVerb<FileInfo[]>, IListVerb, IIn<DirectoryInfo>, IUsing<SearchScope>, IWith<FilePattern>, IPipelineProducer<FileInfo[]>
{
    ...
    public ListFilesScoped([In] DirectoryInfo directory, [Using] SearchScope scope, [With] FilePattern? pattern = null)
    {
...
public sealed class ListDirectories : IVerb<DirectoryInfo[]>, IListVerb, IIn<DirectoryInfo>, IUsing<SearchScope>, IPipelineProducer<DirectoryInfo[]>
{
    ...
    public ListDirectories([In] DirectoryInfo directory, [Using] SearchScope scope)
    {
        _directory = directory;
        _scope = scope;
    }
...
public sealed class SaveTextWithMode : IVerb<string>, ISave, IWhat<string>, ITo<FileInfo>, IUsing<FileWriteMode>, IPipelineConsumer<string>, IPipelineProducer<string>
{
    ...
    public SaveTextWithMode([What] string text, [To] FileInfo file, [Using] FileWriteMode mode)
    {
        _text = text;
        _file = file;
        _mode = mode;
    ...
    public async ValueTask<string> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (_mode == FileWriteMode.APPEND)
            await File.AppendAllTextAsync(_file.FullName, _text, cancellationToken).ConfigureAwait(false);
        else if (_mode == FileWriteMode.ATOMIC)
        {
            string temp = _file.FullName + "." + Guid.NewGuid().ToString("N") + ".tmp";
            try
            {
                await File.WriteAllTextAsync(temp, _text, cancellationToken).ConfigureAwait(false);
                File.Move(temp, _file.FullName, true);
            }
            finally { if (File.Exists(temp)) File.Delete(temp); }
        }
        else
            await File.WriteAllTextAsync(_file.FullName, _text, cancellationToken).ConfigureAwait(false);
        return _text;
    }
}
...
public sealed class CopyDirectory : IVerb<DirectoryInfo>, ICopy, IWhat<DirectoryInfo>, ITo<DirectoryInfo>, IPipelineConsumer<DirectoryInfo>, IPipelineProducer<DirectoryInfo>
{
    ...
    public CopyDirectory([What] DirectoryInfo source, [To] DirectoryInfo destination)
    {
        _source = source;
    ...
    {
        if (!_source.Exists)
            throw new DirectoryNotFoundException(_source.FullName);
        _destination.Create();
        foreach (DirectoryInfo directory in _source.GetDirectories("*", SearchOption.AllDirectories))
            Directory.CreateDirectory(Path.Combine(_destination.FullName, Path.GetRelativePath(_source.FullName, directory.FullName)));
        foreach (FileInfo file in _source.GetFiles("*", SearchOption.AllDirectories))
        {
            string target = Path.Combine(_destination.FullName, Path.GetRelativePath(_source.FullName, file.FullName));
...
{
    private readonly DirectoryInfo _source; private readonly DirectoryInfo _destination;
    public MoveDirectory([What] DirectoryInfo source, [To] DirectoryInfo destination)
    {
        _source = source;
        _destination = destination;
    }
...
public sealed class DeleteDirectory : IVerb<bool>, IDelete, IAt<DirectoryInfo>, IPipelineProducer<bool>
{
    ...
    {
        bool existed = _directory.Exists;
        if (existed)
