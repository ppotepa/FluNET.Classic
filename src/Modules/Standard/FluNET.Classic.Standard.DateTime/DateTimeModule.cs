...
public enum TimeZoneTarget
{
    UTC, LOCAL
}
public enum DateBoundary
{
    START_OF_DAY, END_OF_DAY, START_OF_WEEK, END_OF_WEEK, START_OF_MONTH, END_OF_MONTH, START_OF_YEAR, END_OF_YEAR
}
...
public sealed class TimeZoneSpec
{
    ...
    {
        result = null;
        if (value.Equals("UTC", StringComparison.OrdinalIgnoreCase) || value.Equals("LOCAL", StringComparison.OrdinalIgnoreCase))
            return false;
        try
        {
            result = new(TimeZoneInfo.FindSystemTimeZoneById(value));
            return true;
        }
...
public sealed class CreateDateRange : IVerb<DateRange>, ICreate, IFrom<DateTimeOffset>, ITo<DateTimeOffset>, IPipelineProducer<DateRange>
{
    ...
    public CreateDateRange([From] DateTimeOffset start, [To] DateTimeOffset end)
    {
        _start = start;
        _end = end;
    }
...
{
    private readonly DateOnly _value; private readonly string _format; public FormatDate([What] DateOnly value, [Using] string format)
    {
        _value = value;
        _format = format;
    }
...
{
    private readonly TimeOnly _value; private readonly string _format; public FormatTime([What] TimeOnly value, [Using] string format)
    {
        _value = value;
        _format = format;
    }
...
{
    private readonly DateTimeOffset _value; private readonly string _format; public FormatDateTime([What] DateTimeOffset value, [Using] string format)
    {
        _value = value;
        _format = format;
    }
...
{
    private readonly DateTimeOffset _value; private readonly TimeSpan _duration; public TransformDateTimeByDuration([What] DateTimeOffset value, [Using] TimeSpan duration)
    {
        _value = value;
        _duration = duration;
    }
...
{
    private readonly DateTimeOffset _value; private readonly DateBoundary _boundary; public TransformDateBoundary([What] DateTimeOffset value, [Using] DateBoundary boundary)
    {
        _value = value;
        _boundary = boundary;
    }
...
{
    private readonly DateTimeOffset _deadline; private readonly IClock _clock; public WaitUntil([Until] DateTimeOffset deadline, [FromServices] IClock clock)
    {
        _deadline = deadline;
        _clock = clock;
    }
    ...
    {
        TimeSpan delay = _deadline - _clock.Now;
