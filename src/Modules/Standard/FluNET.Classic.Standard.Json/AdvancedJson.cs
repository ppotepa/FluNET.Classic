...
public enum JsonFormatting { COMPACT, INDENTED }
...
[Verb("GET"), Qualifier("JSON"), ExecutionTrait(ExecutionTrait.Pure)]
public sealed class GetJsonAtPath : IVerb<JsonNode?>, IGet, IFrom<JsonNode>, IAt<JsonPath>, IPipelineProducer<JsonNode?>
{
    private readonly JsonNode _node; private readonly JsonPath _path; public GetJsonAtPath([From] JsonNode node, [At] JsonPath path) { _node = node; _path = path; }
    public ValueTask<JsonNode?> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default) { JsonNode? current = _node; foreach (string segment in _path.Segments) current = current switch { JsonObject obj when obj.TryGetPropertyValue(segment, out JsonNode? value) => value, JsonArray array when int.TryParse(segment, out int index) && index >= 0 && index < array.Count => array[index], _ => null }; return ValueTask.FromResult(current); }
}
...
{
    private readonly JsonNode _node; private readonly JsonPath _path; public GetJsonTextAtPath([From] JsonNode node, [At] JsonPath path) { _node = node; _path = path; }
    public async ValueTask<string?> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default) { JsonNode? value = await new GetJsonAtPath(_node, _path).ExecuteAsync(context, cancellationToken).ConfigureAwait(false); return value is JsonValue ? value.ToString() : value?.ToJsonString(); }
}

[Verb("LIST"), Qualifier("PROPERTIES"), ExecutionTrait(ExecutionTrait.Pure)]
...
[Verb("LIST"), Qualifier("ITEMS"), ExecutionTrait(ExecutionTrait.Pure)]
...
[Verb("CHECK"), Qualifier("JSON"), ExecutionTrait(ExecutionTrait.Pure)]
...
{
    private readonly JsonNode _node; private readonly JsonShape _shape; public CheckJsonShape([What] JsonNode node, [Using] JsonShape shape) { _node = node; _shape = shape; }
    ...
    {
        var errors = new List<string>(); if (_node is not JsonObject obj) errors.Add("Root value is not an object."); else foreach ((string name, JsonValueKind kind) in _shape.Properties) { if (!obj.TryGetPropertyValue(name, out JsonNode? value) || value is null) { if (_shape.RequireAll) errors.Add($"Missing property '{name}'."); continue; } using JsonDocument doc = JsonDocument.Parse(value.ToJsonString()); if (doc.RootElement.ValueKind != kind) errors.Add($"Property '{name}' is {doc.RootElement.ValueKind}, expected {kind}."); }
        return ValueTask.FromResult(new JsonValidationResult(errors.Count == 0, errors));
    }
}

[Verb("FORMAT"), Qualifier("JSON"), ExecutionTrait(ExecutionTrait.Pure)]
...
{
    private readonly JsonNode _node; private readonly JsonFormatting _formatting; public FormatJsonUsing([What] JsonNode node, [Using] JsonFormatting formatting) { _node = node; _formatting = formatting; }
