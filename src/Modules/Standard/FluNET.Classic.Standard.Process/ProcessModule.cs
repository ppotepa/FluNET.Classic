...
public sealed class RunProcess : IVerb<ProcessResult>, IRun, IWhat<ProcessSpec>, IWith<string>, IPipelineProducer<ProcessResult>
{
    ...
    {
        var startInfo = BuildStartInfo(_spec, _arguments, redirect: true); using var process = new System.Diagnostics.Process { StartInfo = startInfo }; var stopwatch = Stopwatch.StartNew(); if (!process.Start()) throw new InvalidOperationException($"Could not start process '{_spec.FileName}'.");
        Task<string> stdout = process.StandardOutput.ReadToEndAsync(); Task<string> stderr = process.StandardError.ReadToEndAsync(); using CancellationTokenSource? timeout = _spec.Timeout is { } duration && duration > TimeSpan.Zero ? CancellationTokenSource.CreateLinkedTokenSource(cancellationToken) : null; if (timeout is not null) timeout.CancelAfter(_spec.Timeout!.Value); CancellationToken token = timeout?.Token ?? cancellationToken;
        try { await process.WaitForExitAsync(token).ConfigureAwait(false); } catch (OperationCanceledException) { if (!process.HasExited) process.Kill(true); throw; }
        await Task.WhenAll(stdout, stderr).ConfigureAwait(false); stopwatch.Stop(); return new(process.ExitCode, stdout.Result, stderr.Result, stopwatch.Elapsed);
    }
...
[Verb("RUN"), Qualifier("PROCESS"), RequiresCapability(StandardCapabilities.ProcessExecute), ExecutionTrait(ExecutionTrait.SideEffecting), ExecutionTrait(ExecutionTrait.LongRunning)]
public sealed class RunBackgroundProcess : IVerb<ProcessHandle>, IRun, IWhat<ProcessSpec>, IUsing<ProcessRunMode>, IWith<string>, IPipelineProducer<ProcessHandle>
{
    ...
    public RunBackgroundProcess([What] ProcessSpec spec, [Using] ProcessRunMode mode, [With] string? arguments = null) { _spec = spec; _mode = mode; _arguments = arguments; }
    public ValueTask<ProcessHandle> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default)
    {
        if (_mode != ProcessRunMode.BACKGROUND) throw new NotSupportedException(_mode.ToString()); var process = new System.Diagnostics.Process { StartInfo = RunProcess.BuildStartInfo(_spec, _arguments, redirect: false), EnableRaisingEvents = false }; if (!process.Start()) throw new InvalidOperationException($"Could not start process '{_spec.FileName}'."); int id = process.Id; process.Dispose(); return ValueTask.FromResult(new ProcessHandle(id, _spec, DateTimeOffset.UtcNow));
    }
}

[Qualifier("STDOUT")] public sealed class GetStandardOutput : Get<string, ProcessResult> { public GetStandardOutput([What] string what, [From] ProcessResult from) : base(what, from) { } protected override ValueTask<string> ActAsync(ProcessResult from, CancellationToken cancellationToken) => ValueTask.FromResult(from.StdOut); }
[Qualifier("STDERR")] public sealed class GetStandardError : Get<string, ProcessResult> { public GetStandardError([What] string what, [From] ProcessResult from) : base(what, from) { } protected override ValueTask<string> ActAsync(ProcessResult from, CancellationToken cancellationToken) => ValueTask.FromResult(from.StdErr); }
[Qualifier("EXITCODE")] public sealed class GetExitCode : Get<int, ProcessResult> { public GetExitCode([What] int what, [From] ProcessResult from) : base(what, from) { } protected override ValueTask<int> ActAsync(ProcessResult from, CancellationToken cancellationToken) => ValueTask.FromResult(from.ExitCode); }

[Verb("LIST"), Qualifier("PROCESSES"), RequiresCapability(StandardCapabilities.ProcessInspect)]
...
[Verb("STOP"), RequiresCapability(StandardCapabilities.ProcessTerminate)]
...
public sealed class WaitForProcess : IVerb<ProcessInfo>, IWait, IFor<ProcessInfo>, IPipelineProducer<ProcessInfo>
{
    ...
    public async ValueTask<ProcessInfo> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default) { try { using System.Diagnostics.Process process = System.Diagnostics.Process.GetProcessById(_process.Id); if (!process.HasExited) await process.WaitForExitAsync(cancellationToken).ConfigureAwait(false); } catch (ArgumentException) { } return _process; }
}

[Verb("WAIT"), Qualifier("PROCESS"), RequiresCapability(StandardCapabilities.ProcessInspect), ExecutionTrait(ExecutionTrait.LongRunning)]
public sealed class WaitForProcessHandle : IVerb<ProcessHandle>, IWait, IFor<ProcessHandle>, IPipelineProducer<ProcessHandle>
{
    ...
    public async ValueTask<ProcessHandle> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default) { try { using System.Diagnostics.Process process = System.Diagnostics.Process.GetProcessById(_process.Id); if (!process.HasExited) await process.WaitForExitAsync(cancellationToken).ConfigureAwait(false); } catch (ArgumentException) { } return _process; }
}
