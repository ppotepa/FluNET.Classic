...
public sealed record SqlResult(int AffectedRows, IReadOnlyList<SqlRow> Rows) : IOkState { public bool IsOk => AffectedRows >= 0; }
...
[Verb("GET"), Qualifier("ROWS"), RequiresCapability(StandardCapabilities.SqlRead), ExecutionTrait(ExecutionTrait.Retryable)]
public sealed class GetSqlRows : IVerb<SqlRow[]>, IGet, IFrom<SqlQuery>, IUsing<DatabaseConnection>, IPipelineProducer<SqlRow[]>
{
    ...
    public GetSqlRows([From] SqlQuery query, [Using] DatabaseConnection connection, [FromServices] ISqlExecutor executor) { _query = query; _connection = connection; _executor = executor; }
...
[Verb("RUN"), Qualifier("RESULT"), RequiresCapability(StandardCapabilities.SqlWrite), ExecutionTrait(ExecutionTrait.SideEffecting), ExecutionTrait(ExecutionTrait.Transactional)]
public sealed class RunSqlCommand : IVerb<SqlResult>, IRun, IWhat<SqlCommand>, IUsing<DatabaseConnection>, IPipelineProducer<SqlResult>
{
    ...
    public RunSqlCommand([What] SqlCommand command, [Using] DatabaseConnection connection, [FromServices] ISqlExecutor executor) { _command = command; _connection = connection; _executor = executor; }
