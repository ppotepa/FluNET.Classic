...
public enum CompressionFormat { ZIP }
...
[Verb("CREATE"), Qualifier("ARCHIVE"), RequiresCapability(StandardCapabilities.FileSystemRead)]
public sealed class CreateArchive : IVerb<ArchiveDocument>, ICreate, IFrom<FileInfo[]>, IUsing<CompressionFormat>, IPipelineProducer<ArchiveDocument>
{
    ...
    public CreateArchive([From] FileInfo[] files, [Using] CompressionFormat format) { _files = files; _format = format; }
...
[Verb("LIST"), Qualifier("ENTRIES"), ExecutionTrait(ExecutionTrait.Pure)]
public sealed class ListArchiveEntries : IVerb<ArchiveEntry[]>, IListVerb, IFrom<ArchiveDocument>, IPipelineConsumer<ArchiveDocument>, IPipelineProducer<ArchiveEntry[]>
{
    ...
    {
        using var input = new MemoryStream(_archive.Data); using var zip = new ZipArchive(input, ZipArchiveMode.Read);
        var result = new List<ArchiveEntry>();
        foreach (ZipArchiveEntry entry in zip.Entries) { await using Stream stream = entry.Open(); await using var data = new MemoryStream(); await stream.CopyToAsync(data, cancellationToken).ConfigureAwait(false); result.Add(new(entry.FullName, data.ToArray(), entry.Length)); }
        return result.ToArray();
    }
}

[Verb("GET"), Qualifier("BINARY"), ExecutionTrait(ExecutionTrait.Pure)]
