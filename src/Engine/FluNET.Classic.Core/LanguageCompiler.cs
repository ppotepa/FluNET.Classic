...
public sealed class LanguageCompiler
{
    ...
    private VerbImplementationDescriptor? CompileVerb(Type type, ICollection<LanguageDiagnostic> diagnostics)
    {
        ...
        if (patterns.All(pattern => pattern.Roles.Count == 0) && !ImplementsGeneric(type, typeof(IContextQuery<>)))
            diagnostics.Add(new("FLU-LANG-015", $"Verb '{type.FullName}' has no constructor with language roles; use IContextQuery<TResult> for a context-backed query.", LanguageDiagnosticSeverity.Error, type));
