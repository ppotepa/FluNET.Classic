...
public sealed class SemanticBinder
{
    ...
    {
        _diagnostics.Clear(); _scriptCallables.Clear(); _recordSchemas.Clear(); _activeReturnType = null;
        foreach (RecordDefinitionNode record in script.Statements.OfType<RecordDefinitionNode>()) RegisterRecord(record);
        foreach (DefinitionNode definition in script.Statements.OfType<DefinitionNode>()) RegisterDefinition(definition);
        foreach (DefinitionNode definition in script.Statements.OfType<DefinitionNode>()) BindDefinition(definition);
        DiagnoseDefinitionCycles();
        var symbols = new SymbolScope(null); if (initialVariables is not null) foreach ((string name, Type type) in initialVariables) symbols.Define(name, type);
        BoundStatement[] statements = script.Statements.Where(x => x is not DefinitionNode and not RecordDefinitionNode).Select(x => BindStatement(x, symbols)).Where(x => x is not null).Cast<BoundStatement>().ToArray(); return new(statements, _diagnostics.ToArray(), _scriptCallables);
    }
    ...
    {
        if (_recordSchemas.ContainsKey(record.Name)) { _diagnostics.Add(new("FLU-BIND-230", $"Record '{record.Name}' is already defined.", record.Span)); return; }
        var fields = new List<FluRecordField>();
        foreach (RecordFieldNode field in record.Fields)
        {
            if (!TryResolveType(field.TypeName, out Type? type)) { _diagnostics.Add(new("FLU-BIND-231", $"Unknown type '{field.TypeName}' in record '{record.Name}'.", field.Span)); type = typeof(object); }
            fields.Add(new(field.Name, type!));
        }
        try { _recordSchemas.Add(record.Name, new FluRecordSchema(record.Name, fields)); }
    ...
    {
        if (!_scriptCallables.TryGetValue(definition.Name, out BoundScriptCallable? callable)) return;
        var symbols = new SymbolScope(null);
        foreach (BoundScriptParameter parameter in callable.Parameters) symbols.Define(parameter.Name, parameter.Type);
        Type? previous = _activeReturnType; _activeReturnType = callable.ReturnType;
    ...
    private void DiagnoseDefinitionCycles()
    {
        ...
        foreach (BoundScriptCallable callable in _scriptCallables.Values) Visit(callable);
        ...
        void Visit(BoundScriptCallable callable)
        {
            ...
            if (!visited.Add(callable)) return;
            active.Add(callable);
            foreach (BoundScriptCallable child in Calls(callable.Body)) Visit(child);
        ...
        foreach (BoundStatement statement in block.Statements)
        {
            switch (statement)
            {
                case BoundPipeline pipeline:
                    foreach (BoundScriptCall call in pipeline.Stages.OfType<BoundScriptCall>()) yield return call.Callable;
                    break;
                case BoundIf conditional:
                    foreach (BoundScriptCallable call in Calls(conditional.Then)) yield return call;
                    if (conditional.Else is not null) foreach (BoundScriptCallable call in Calls(conditional.Else)) yield return call;
                    break;
                case BoundForEach loop:
                    foreach (BoundScriptCallable call in Calls(loop.Body)) yield return call;
                    break;
                case BoundTry @try:
                    foreach (BoundScriptCallable call in Calls(@try.Body)) yield return call;
                    if (@try.Failure is not null) foreach (BoundScriptCallable call in Calls(@try.Failure)) yield return call;
                    if (@try.Finally is not null) foreach (BoundScriptCallable call in Calls(@try.Finally)) yield return call;
                    break;
            }
        }
    }
    ...
    {
        if (_activeReturnType is null) { _diagnostics.Add(new("FLU-BIND-203", "RETURN is only valid inside a task or function.", node.Span)); return null; }
        if (node.Value is null)
        {
            if (_activeReturnType != typeof(void)) _diagnostics.Add(new("FLU-BIND-204", $"This definition must return {Friendly(_activeReturnType)}.", node.Span));
    ...
    private BoundPipeline BindPipeline(PipelineNode pipeline, SymbolScope symbols)
    {
        ...
        foreach (PipelineStageNode stage in pipeline.Stages)
        {
            ...
            if (bound is BoundSentence boundSentence) RegisterOutputs(boundSentence, symbols);
            if (bound is BoundScriptCall scriptCall && scriptCall.ResultAlias is { Length: > 0 }) DefineBinding(symbols, scriptCall.ResultAlias, scriptCall.ResultType, scriptCall.Span);
            if (bound is BoundRecordCreate recordCreate && recordCreate.ResultAlias is { Length: > 0 }) DefineBinding(symbols, recordCreate.ResultAlias, recordCreate.ResultType, recordCreate.Span);
            if (bound is BoundFilter { ResultAlias: { Length: > 0 } filterAlias }) DefineBinding(symbols, filterAlias, bound.ResultType, bound.Span);
            if (bound is BoundCheck { ResultAlias: { Length: > 0 } checkAlias }) DefineBinding(symbols, checkAlias, typeof(bool), bound.Span);
            if (bound is BoundCollection { ResultAlias: { Length: > 0 } collectionAlias }) DefineBinding(symbols, collectionAlias, bound.ResultType, bound.Span);
        }
    ...
    private BoundStage? BindSentence(SentenceNode sentence, SymbolScope symbols, Type? pipelineType)
    {
        ...
        Candidate selected = ordered[0]; return new BoundSentence(verb, selected.Implementation, selected.Pattern, selected.Roles, sentence.ResultAlias, selected.Cost, sentence.Span);
    }
    ...
    private BoundRecordCreate BindRecordCreate(SentenceNode sentence, FluRecordSchema schema, SymbolScope symbols)
    {
        ...
        if (values.Length != schema.Fields.Count) _diagnostics.Add(new("FLU-BIND-233", $"MAKE {schema.Name} requires {schema.Fields.Count} values, got {values.Length}.", sentence.Span));
        ...
        for (int index = 0; index < Math.Min(values.Length, schema.Fields.Count); index++)
        {
            ...
            else bound.Add(new(field, value!));
        }
    ...
    private BoundScriptCall? BindScriptCall(SentenceNode sentence, BoundScriptCallable callable, SymbolScope symbols, Type? pipelineType)
    {
        ...
        foreach (BoundScriptParameter parameter in callable.Parameters)
        {
            ...
            if (expression is null && parameter.RoleName.Equals("WHAT", StringComparison.OrdinalIgnoreCase) && pipelineType is not null && !pipelineUsed && _conversions.TryPlan(pipelineType, parameter.Type, out ConversionPlan? pipelinePlan))
            {
                ...
                if (pipelinePlan!.Kind != ConversionKind.Exact) pipeline = new BoundConversionValue(pipeline, parameter.Type, pipelinePlan.Kind, pipelinePlan.Cost, sentence.Span, pipelinePlan);
                arguments.Add(new(parameter, pipeline)); pipelineUsed = true; continue;
            }
            ...
            if (values is { Count: > 0 }) _diagnostics.Add(new("FLU-BIND-212", $"Call to {callable.Name} has too many values for {parameter.RoleName}.", values.Peek().Span));
        }
    ...
    private bool TryBindValue(ExpressionNode expression, Type? expected, RoleDirection direction, string verb, string? qualifier, SymbolScope symbols, out BoundValue? bound, out int cost, string? roleName = null)
    {
        ...
        switch (expression)
        {
            case BinaryExpression or BetweenExpression or UnaryExpression or PredicateExpression:
                BoundExpression expressionValue = BindExpression(expression, symbols, null); bound = new BoundExpressionValue(expressionValue, expression.Span); return ApplyExpected(ref bound, expected, expression.Span, out cost);
            case VariableExpression variable: if (!symbols.TryGet(variable.Name, out Type variableType)) { bound = null; cost = 0; return false; } bound = new BoundVariableValue(variable.Name, variableType, false, variable.Span); return ApplyExpected(ref bound, expected, variable.Span, out cost);
            case PropertyExpression property:
                if (TryBindProperty(property, symbols, out BoundPropertyValue? propertyValue)) { bound = propertyValue; return ApplyExpected(ref bound, expected, property.Span, out cost); }
    ...
    private bool ResolveText(string text, Type expected, TextSpan span, string verb, string? qualifier, string? roleName, ResolutionSourceKind sourceKind, out BoundValue? bound, out int cost)
    {
        ...
        if (resolution.Success) { bound = new BoundConstantValue(resolution.Value, expected, span, ConversionKind.Resolution, 4); cost = 4; return true; }
    ...
    private void RegisterOutputs(BoundSentence sentence, SymbolScope symbols) { foreach (BoundVariableValue variable in sentence.Roles.Where(x => x.Slot.Direction is RoleDirection.Output or RoleDirection.InputOutput).SelectMany(x => x.Values).OfType<BoundVariableValue>().Where(x => x.IsOutput)) DefineBinding(symbols, variable.Name, variable.VariableType, variable.Span); if (sentence.ResultAlias is { Length: > 0 } alias) DefineBinding(symbols, alias, sentence.ResultType, sentence.Span); }
    ...
    private sealed class SymbolScope(SymbolScope? parent)
    {
        ...
        {
            if (_symbols.ContainsKey(name) || (parent is not null && parent.TryGet(name, out _))) return false;
            _symbols[name] = type;
            return true;
        }
