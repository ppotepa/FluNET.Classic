...
public sealed class ClassicParser
{
    ...
    private List<StatementNode> ParseStatements()
    {
        ...
        {
            if (IsWord("ELSE") || IsWord("END") || IsWord("FINALLY") || IsWord("ON") && IsWord("FAILURE", 1)) break;
            StatementNode? statement = ParseStatement();
            if (statement is not null)
            {
                result.Add(statement);
                if (statement is not CommentStatementNode) RequirePeriod();
            }
            else
            {
                SkipToBoundary();
                if (Current.Kind == TokenKind.RightBrace) Advance();
            }
            SkipNewLines();
        }
        return result;
    ...
    private StatementNode? ParseStatement()
    {
        ...
        if (IsWord("TRY")) return ParseTry();
        if (IsWord("DEFINE")) return IsWord("RECORD", 1) ? ParseRecordDefinition() : ParseDefinition();
        if (IsWord("RETURN")) return ParseReturn();
    ...
    private ForEachNode ParseForEach()
    {
        ...
        ExpectWord("IN", "FLU-SYN-112", "FOR EACH requires IN."); ExpressionNode source = ParseExpressionUntil("DO");
        ...
        if (Current.Kind == TokenKind.Comma)
        {
            ...
            {
                Advance();
                if (Current.Kind == TokenKind.Number) { parallelism = Convert.ToInt32(Current.Value, CultureInfo.InvariantCulture); Advance(); }
                else _diagnostics.Add(new("FLU-SYN-118", "PARALLEL requires a positive worker count.", Current.Span));
                if (Current.Kind != TokenKind.Comma) _diagnostics.Add(new("FLU-SYN-119", "PARALLEL requires ', DO'.", Current.Span));
                else Advance();
                SkipNewLines(); ExpectWord("DO", "FLU-SYN-113", "FOR EACH requires ', DO'."); SkipNewLines();
            }
            else _position = saved;
        }
        if (parallelism is <= 0) _diagnostics.Add(new("FLU-SYN-118", "PARALLEL requires a positive worker count.", Previous.Span));
        if (parallelism is null) ConsumeDo("FLU-SYN-113", "FOR EACH requires ', DO'.");
    ...
    private DefinitionNode ParseDefinition()
    {
        ...
        if (!isFunction && !IsWord("TASK")) _diagnostics.Add(new("FLU-SYN-140", "DEFINE requires FUNCTION or TASK.", Current.Span));
        else Advance();
        SyntaxToken name = Current;
        if (name.Kind != TokenKind.Word) _diagnostics.Add(new("FLU-SYN-141", "A definition requires a verb name.", name.Span)); else Advance();
        string? qualifier = null;
        if (Current.Kind == TokenKind.Word && !IsWord("WHAT") && !IsWord("RETURNING") && !IsWord("DO")) { qualifier = Current.Text; Advance(); }
        ...
        while (!IsWord("RETURNING") && !IsWord("DO") && Current.Kind != TokenKind.End)
        {
            ...
            Advance(); ExpectWord("AS", "FLU-SYN-144", "Definition parameters require AS TYPE.");
            SyntaxToken type = Current;
            if (type.Kind != TokenKind.Word) _diagnostics.Add(new("FLU-SYN-145", "Definition parameters require a type name.", type.Span)); else Advance();
        ...
        if (returnType.Kind != TokenKind.Word) _diagnostics.Add(new("FLU-SYN-147", "Definition requires a return type.", returnType.Span)); else Advance();
    ...
    {
        int start = Advance().Span.Start; ExpectWord("RECORD", "FLU-SYN-150", "DEFINE requires RECORD.");
        SyntaxToken name = Current;
        if (name.Kind != TokenKind.Word) _diagnostics.Add(new("FLU-SYN-151", "A record requires a name.", name.Span)); else Advance();
        ...
        while (Current.Kind is not TokenKind.End and not TokenKind.Period)
        {
            ...
            if (field.Kind != TokenKind.Word) { _diagnostics.Add(new("FLU-SYN-152", "Expected a record field name.", field.Span)); Advance(); continue; }
            Advance(); ExpectWord("AS", "FLU-SYN-153", "Record fields require AS TYPE.");
            SyntaxToken type = Current;
            if (type.Kind != TokenKind.Word) _diagnostics.Add(new("FLU-SYN-154", "Record fields require a type name.", type.Span)); else Advance();
    ...
    private CollectionStageNode ParseIntrinsic(IntrinsicDescriptor intrinsic)
    {
        ...
        string? alias = ParseOptionalResultAlias(); int end = alias is not null ? Previous.Span.End : strategy?.Span.End ?? argument?.Span.End ?? source?.Span.End ?? operation.Span.End; return new(name, source, argument, strategy, alias, TextSpan.FromBounds(start, end));
    }
    ...
    private SentenceNode? ParseSentence()
    {
        ...
        Flush(); int end = alias is not null ? Previous.Span.End : clauses.LastOrDefault()?.Span.End ?? verbToken.Span.End; return new(verbToken.Text, qualifier, clauses, alias, TextSpan.FromBounds(verbToken.Span.Start, end));
    ...
    private bool CommaStartsPipelineContinuation() { if (Current.Kind != TokenKind.Comma) return false; int offset = 1; while (Peek(offset).Kind == TokenKind.NewLine) offset++; return IsWordAt("THEN", offset); }
    ...
    private void ExpectWord(string word, string code, string message) { if (IsWord(word)) Advance(); else _diagnostics.Add(new(code, message, Current.Span)); }
    private void ConsumeThen(string code, string message) { if (Current.Kind != TokenKind.Comma) _diagnostics.Add(new("FLU-SYN-103", "A comma is required before THEN.", Current.Span)); else Advance(); SkipNewLines(); ExpectWord("THEN", code, message); SkipNewLines(); }
    private void ConsumeDo(string code, string message) { if (Current.Kind != TokenKind.Comma) _diagnostics.Add(new("FLU-SYN-104", "A comma is required before DO.", Current.Span)); else Advance(); SkipNewLines(); ExpectWord("DO", code, message); SkipNewLines(); }
    private void ExpectEnd(string owner, string code, string message) { if (!IsWord("END")) { _diagnostics.Add(new(code, message, Current.Span)); return; } Advance(); ExpectWord(owner, code, message); SkipNewLines(); }
    private void RequirePeriod()
    {
        SkipNewLines();
        if (Current.Kind == TokenKind.Period) { Advance(); return; }
    ...
    private void SkipToBoundary() { while (Current.Kind is not TokenKind.End and not TokenKind.Period and not TokenKind.Comment and not TokenKind.RightBrace) Advance(); }
