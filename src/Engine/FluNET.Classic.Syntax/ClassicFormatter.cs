...
public sealed class ClassicFormatter
{
    ...
    private string FormatSentence(SentenceNode sentence)
    {
        ...
        foreach (ClauseNode clause in sentence.Clauses)
        {
            ...
            if (clause.RoleName.Equals("WHAT", StringComparison.OrdinalIgnoreCase))
                ...
                sb.Append(' ').Append(clause.RoleName.ToUpperInvariant()).Append(' ').Append(string.Join(", ", clause.Values.Select(FormatExpression)));
        }
        if (!string.IsNullOrWhiteSpace(sentence.ResultAlias))
            sb.Append(" INTO [").Append(sentence.ResultAlias).Append(']');
        return sb.ToString();
    }
    private string FormatFilter(FilterStageNode filter)
    {
        ...
        if (filter.Source is not null)
            sb.Append(' ').Append(FormatExpression(filter.Source));
        sb.Append(" WHERE ").Append(FormatExpression(filter.Predicate));
        if (!string.IsNullOrWhiteSpace(filter.ResultAlias))
            sb.Append(" INTO [").Append(filter.ResultAlias).Append(']');
        return sb.ToString();
    }
    ...
    private string FormatCollection(CollectionStageNode node)
    {
        ...
        switch (intrinsic?.Syntax)
        {
            case IntrinsicSyntaxKind.CollectionBy:
                if (node.Source is not null)
                    sb.Append(' ').Append(FormatExpression(node.Source));
            ...
            case IntrinsicSyntaxKind.CollectionAmountFrom:
                ...
                if (node.Source is not null)
                    sb.Append(" FROM ").Append(FormatExpression(node.Source));
                break;
        ...
        if (!string.IsNullOrWhiteSpace(node.ResultAlias))
            sb.Append(" INTO [").Append(node.ResultAlias).Append(']');
        return sb.ToString();
    }
    ...
    private string FormatForEach(ForEachNode loop)
    {
        ...
        if (loop.Parallelism is { } parallelism)
            sb.Append(", PARALLEL ").Append(parallelism);
        sb.AppendLine(", DO");
    ...
    private string FormatDefinition(DefinitionNode definition)
    {
        ...
        if (definition.Qualifier is not null)
            sb.Append(' ').Append(definition.Qualifier.ToUpperInvariant());
        foreach (DefinitionParameterNode parameter in definition.Parameters)
            sb.Append(", ").Append(parameter.RoleName).Append(" [").Append(parameter.Name).Append("] AS ").Append(parameter.TypeName.ToUpperInvariant());
    ...
    {
        if (_language is not null && _language.TryGetOperator(surface, out OperatorDescriptor descriptor))
            return descriptor;
    ...
    {
        if (_language is not null && _language.TryGetPredicate(surface, out PredicateDescriptor descriptor))
            return descriptor;
    ...
    {
        if (_language is not null && _language.TryGetIntrinsic(surface, out IntrinsicDescriptor descriptor))
            return descriptor;
        return null;
    }
