...
public sealed class BoundExecutor
{
    ...
    public BoundExecutor(ValueConversionRegistry conversions, PredicateRegistry predicates, ICapabilityPolicy capabilities, IServiceProvider? services = null, ExecutionPolicy? policy = null, OperatorEvaluatorRegistry? operatorEvaluators = null, IExecutionObserver? observer = null) { _conversions = conversions; _predicates = predicates; _capabilities = capabilities; _services = services; _policy = policy ?? new ExecutionPolicy(); _operatorEvaluators = operatorEvaluators ?? new OperatorEvaluatorRegistry(); _observer = observer ?? NullExecutionObserver.Instance; }
    ...
    private async ValueTask ExecuteStatement(BoundStatement statement, RuntimeState state, CancellationToken ct)
    {
        ct.ThrowIfCancellationRequested(); switch (statement)
        {
            case BoundPipeline pipeline:
                foreach (BoundStage stage in pipeline.Stages)
                {
                    ...
                    switch (stage)
                    {
                        ...
                        case BoundScriptCall scriptCall: await ExecuteScriptCall(scriptCall, state, ct).ConfigureAwait(false); break;
                        case BoundRecordCreate recordCreate: ExecuteRecordCreate(recordCreate, state); break;
            ...
            case BoundTry @try:
                ...
                { if (@try.Finally is not null) await ExecuteBlock(@try.Finally, state, ct).ConfigureAwait(false); }
                break;
    ...
    private async ValueTask ExecuteScriptCall(BoundScriptCall call, RuntimeState caller, CancellationToken ct)
    {
        ...
        foreach (BoundScriptArgument argument in call.Arguments) nested.SetVariable(argument.Parameter.Name, Materialize(argument.Value, caller));
        object? result = null;
        try
        {
        ...
        if (call.ResultAlias is { Length: > 0 } alias) caller.SetVariable(alias, result);
    }

    private void ExecuteRecordCreate(BoundRecordCreate create, RuntimeState state)
    {
        var values = new Dictionary<string, object?>(StringComparer.OrdinalIgnoreCase);
        foreach (BoundRecordFieldValue field in create.Fields) values[field.Field.Name] = Materialize(field.Value, state);
        var record = new FluRecord(create.Schema, values);
        state.PipelineValue = record;
        if (create.ResultAlias is { Length: > 0 } alias) state.SetVariable(alias, record);
    }
    ...
    private async ValueTask ExecuteParallelLoop(BoundForEach loop, RuntimeState parent, IReadOnlyList<object?> items, CancellationToken ct)
    {
        ...
        Task[] tasks = items.Select(async item =>
        {
            ...
            try
            {
                ...
                foreach ((string name, object? value) in parent.Variables) iteration.SetVariable(name, value);
    ...
    private async ValueTask ExecuteSentence(BoundSentence sentence, RuntimeState state, CancellationToken ct)
    {
        ...
        for (int attempt = 1; attempt <= attempts; attempt++) { used = attempt; try { using CancellationTokenSource? timeout = CreateTimeout(ct, _policy.TimeoutFor(sentence.Implementation.Traits)); CancellationToken token = timeout?.Token ?? ct; object? result = await InvokeSentenceAttempt(sentence, args, state, token).ConfigureAwait(false); state.PipelineValue = result; StoreOutputs(sentence, result, state); if (sentence.ResultAlias is { Length: > 0 } alias) state.SetVariable(alias, result); timer.Stop(); _trace.Add(new(++_sequence, "sentence", sentence.Verb.Name, sentence.Implementation.ImplementationType.FullName, started, timer.Elapsed, true, used, sentence.ResultType.FullName, sentence.Implementation.Capabilities, sentence.Implementation.Traits)); await PublishAsync(new ExecutionEvent(++_sequence, ExecutionEventKind.StageCompleted, Verb: sentence.Verb.Name, Implementation: sentence.Implementation.ImplementationType.FullName, Success: true, Attempts: used, Duration: timer.Elapsed, Detail: sentence.Verb.Name)); return; } catch (Exception ex) when (attempt < attempts && ex is not OperationCanceledException && ex is not UnauthorizedAccessException) { last = ex; if (_policy.RetryDelay > TimeSpan.Zero) await Task.Delay(_policy.RetryDelay, ct).ConfigureAwait(false); } catch (Exception ex) { last = ex; break; } }
        timer.Stop(); _trace.Add(new(++_sequence, "sentence", sentence.Verb.Name, sentence.Implementation.ImplementationType.FullName, started, timer.Elapsed, false, used, sentence.ResultType.FullName, sentence.Implementation.Capabilities, sentence.Implementation.Traits, last?.Message)); await PublishAsync(new ExecutionEvent(++_sequence, ExecutionEventKind.StageCompleted, Verb: sentence.Verb.Name, Implementation: sentence.Implementation.ImplementationType.FullName, Success: false, Attempts: used, Duration: timer.Elapsed, Detail: sentence.Verb.Name, Error: last?.Message)); throw last ?? new InvalidOperationException("Execution failed.");
    }
    private async ValueTask PublishAsync(ExecutionEvent item)
    {
        try { await _observer.OnEventAsync(item).ConfigureAwait(false); }
        catch { }
    }
    ...
    private static IEnumerable<string> CollectCapabilities(BoundStatement statement) { switch (statement) { case BoundPipeline pipeline: foreach (BoundStage stage in pipeline.Stages) { if (stage is BoundSentence sentence) foreach (string capability in sentence.Implementation.Capabilities) yield return capability; if (stage is BoundScriptCall scriptCall) foreach (BoundStatement child in scriptCall.Callable.Body.Statements) foreach (string capability in CollectCapabilities(child)) yield return capability; foreach (string capability in CollectExpressionCapabilities(stage)) yield return capability; } break; case BoundIf conditional: foreach (string capability in CollectExpressionCapabilities(conditional.Condition)) yield return capability; foreach (BoundStatement child in conditional.Then.Statements) foreach (string capability in CollectCapabilities(child)) yield return capability; if (conditional.Else is not null) foreach (BoundStatement child in conditional.Else.Statements) foreach (string capability in CollectCapabilities(child)) yield return capability; break; case BoundForEach loop: foreach (BoundStatement child in loop.Body.Statements) foreach (string capability in CollectCapabilities(child)) yield return capability; break; case BoundTry @try: foreach (BoundStatement child in @try.Body.Statements) foreach (string capability in CollectCapabilities(child)) yield return capability; if (@try.Failure is not null) foreach (BoundStatement child in @try.Failure.Statements) foreach (string capability in CollectCapabilities(child)) yield return capability; if (@try.Finally is not null) foreach (BoundStatement child in @try.Finally.Statements) foreach (string capability in CollectCapabilities(child)) yield return capability; break; } }
