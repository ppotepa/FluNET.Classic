...
public sealed class BoundExecutor
{
    ...
    public BoundExecutor(ValueConversionRegistry conversions, PredicateRegistry predicates, ICapabilityPolicy capabilities, IServiceProvider? services = null, ExecutionPolicy? policy = null, OperatorEvaluatorRegistry? operatorEvaluators = null, IExecutionObserver? observer = null)
    {
        ...
        _operatorEvaluators = operatorEvaluators ?? new OperatorEvaluatorRegistry();
        ...
        switch (statement)
        {
            case BoundPipeline pipeline:
                foreach (BoundStage stage in pipeline.Stages)
                {
                    ...
                    switch (stage)
                    {
                        case BoundSentence sentence:
                            await ExecuteSentence(sentence, state, ct).ConfigureAwait(false);
                            break;
                        case BoundScriptCall scriptCall:
                            await ExecuteScriptCall(scriptCall, state, ct).ConfigureAwait(false);
                            break;
                        case BoundRecordCreate recordCreate:
                            ExecuteRecordCreate(recordCreate, state);
                            break;
                        case BoundFilter filter:
                            await ExecuteFilterAsync(filter, state, ct).ConfigureAwait(false);
                            break;
                        case BoundCheck check:
                            ExecuteCheck(check, state);
                            break;
                        case BoundCollection collection:
                            await ExecuteCollectionAsync(collection, state, ct).ConfigureAwait(false);
                            break;
                    }
                }
                break;
            case BoundIf conditional:
                {
                    ...
                    {
                        if (ToBoolean(EvaluateExpression(conditional.Condition, state, null)))
                            await ExecuteBlock(conditional.Then, state, ct).ConfigureAwait(false);
                        else if (conditional.Else is not null)
                            await ExecuteBlock(conditional.Else, state, ct).ConfigureAwait(false);
                        foreach (BoundFlowVariable variable in conditional.PromotedVariables)
                            if (state.TryGetVariable(variable.Name, out object? value))
                                promoted[variable.Name] = value;
                    }
                    foreach ((string name, object? value) in promoted)
            ...
            case BoundForEach loop:
                {
                    ...
                    if (source is IEnumerable enumerable)
                    {
                        foreach (object? item in enumerable)
                            await ExecuteLoopItem(loop, state, item, ct).ConfigureAwait(false);
                    }
                    else if (source is not null && AsyncSequenceAdapter.CanEnumerate(source))
                    {
                        ...
                        throw new InvalidOperationException("FOR EACH source is not enumerable.");
                    state.PipelineValue = null;
                    break;
                }
            case BoundTry @try:
                try
                {
                    await ExecuteBlock(@try.Body, state, ct).ConfigureAwait(false);
                }
                ...
                {
                    await ExecuteBlock(@try.Failure!, state, ct).ConfigureAwait(false);
                }
                finally
                {
                    if (@try.Finally is not null)
                        await ExecuteBlock(@try.Finally, state, ct).ConfigureAwait(false);
                }
                break;
            case BoundReturn @return:
                throw new ReturnSignal(@return.Value is null ? null : Materialize(@return.Value, state));
        }
    }

    private async ValueTask ExecuteScriptCall(BoundScriptCall call, RuntimeState caller, CancellationToken ct)
    {
        var nested = new RuntimeState();
        foreach (BoundScriptArgument argument in call.Arguments)
            nested.SetVariable(argument.Parameter.Name, Materialize(argument.Value, caller));
        object? result = null;
        try
        {
            await ExecuteBlock(call.Callable.Body, nested, ct).ConfigureAwait(false);
            result = nested.PipelineValue;
        }
        catch (ReturnSignal signal)
        {
            result = signal.Value;
        }
        caller.PipelineValue = result;
        if (call.ResultAlias is { Length: > 0 } alias)
            caller.SetVariable(alias, result);
    }

    private void ExecuteRecordCreate(BoundRecordCreate create, RuntimeState state)
    {
        var values = new Dictionary<string, object?>(StringComparer.OrdinalIgnoreCase);
        foreach (BoundRecordFieldValue field in create.Fields)
            values[field.Field.Name] = Materialize(field.Value, state);
        var record = new FluRecord(create.Schema, values);
        state.PipelineValue = record;
        if (create.ResultAlias is { Length: > 0 } alias)
            state.SetVariable(alias, record);
    }
    ...
    private async ValueTask ExecuteBlock(BoundBlock block, RuntimeState state, CancellationToken ct)
    {
        foreach (BoundStatement statement in block.Statements)
            await ExecuteStatement(statement, state, ct).ConfigureAwait(false);
    }
    private async ValueTask ExecuteSentence(BoundSentence sentence, RuntimeState state, CancellationToken ct)
    {
        foreach (string capability in sentence.Implementation.Capabilities)
            if (!_capabilities.IsAllowed(capability))
                throw new UnauthorizedAccessException($"Capability '{capability}' is required by {sentence.Verb.Name}.");
        ...
        foreach (ParameterDescriptor parameter in sentence.Pattern.Constructor.Parameters)
        {
            ...
            args[parameter.Position] = role is null ? parameter.IsOptional ? parameter.DefaultValue : Default(parameter.ParameterType) : MaterializeRole(role, parameter, state);
        }
        ValidateScopedCapabilities(sentence, args);
        int attempts = _policy.AttemptsFor(sentence.Implementation.Traits);
        int used = 0;
        Exception? last = null;
    ...
    {
        if (_capabilities is not IScopedCapabilityPolicy scoped)
            return;
        foreach (string capability in sentence.Implementation.Capabilities)
        {
            if (scoped.IsAllowed(capability, null))
                continue;
            ...
            if (!allowed)
                throw new UnauthorizedAccessException($"Capability '{capability}' is not allowed for resources used by {sentence.Verb.Name}.");
        }
    }
    private static bool ResourceAllowed(IScopedCapabilityPolicy scoped, string capability, object? resource)
    {
        if (resource is null)
            return false;
        if (scoped.IsAllowed(capability, resource))
            return true;
        if (resource is string or byte[] or ReadOnlyMemory<byte>)
            return false;
        if (resource is IEnumerable enumerable)
            foreach (object? item in enumerable)
                if (ResourceAllowed(scoped, capability, item))
                    return true;
        return false;
    }
    ...
    private async ValueTask ExecuteFilterAsync(BoundFilter filter, RuntimeState state, CancellationToken ct)
    {
        ...
        if (source is not null && AsyncSequenceAdapter.CanEnumerate(source))
        {
            ...
            state.PipelineValue = result;
            if (filter.ResultAlias is { Length: > 0 } alias)
                state.SetVariable(alias, result);
            return;
        }
        ...
        foreach (object? item in items)
            if (ToBoolean(EvaluateExpression(filter.Predicate, state, item)))
                matches.Add(item);
        Array array = ToTypedArray(filter.ElementType, matches);
        state.PipelineValue = array;
        if (filter.ResultAlias is { Length: > 0 } syncAlias)
            state.SetVariable(syncAlias, array);
    }
    ...
    private async ValueTask ExecuteCollectionAsync(BoundCollection operation, RuntimeState state, CancellationToken ct)
    {
        ...
        if (source is not null && AsyncSequenceAdapter.CanEnumerate(source))
        {
            ...
            if (operation.ResultAlias is { Length: > 0 } asyncAlias)
                state.SetVariable(asyncAlias, asyncResult);
            return;
        }

        object? result = await ExecuteMaterializingCollectionAsync(operation, source, state, ct).ConfigureAwait(false);
        state.PipelineValue = result;
        if (operation.ResultAlias is { Length: > 0 } alias)
            state.SetVariable(alias, result);
    }
    ...
    {
        if (operation.Semantic == IntrinsicSemanticKind.Custom)
            throw MissingIntrinsicEvaluator(operation);
    ...
    {
        if (operation.Argument is null)
            throw new InvalidOperationException($"{operation.Operation} requires an amount.");
        int amount = Convert.ToInt32(EvaluateExpression(operation.Argument, state, null), CultureInfo.InvariantCulture);
        if (amount < 0)
            throw new InvalidOperationException($"{operation.Operation} requires a non-negative amount, got {amount}.");
        return amount;
    }
    ...
    private object Distinct(List<object?> items, BoundCollection operation, RuntimeState state)
    {
        ...
        foreach (object? item in items)
        {
            ...
            if (keys.Any(existing => EqualsNormalized(existing, key)))
                continue;
            keys.Add(key);
            distinct.Add(item);
        }
    ...
    {
        if (operation.Argument is null)
            throw new InvalidOperationException($"{operation.Operation} requires a typed BY selector.");
        ...
        foreach (object? item in items)
        {
            ...
            if (index < 0)
                groups.Add((key, new List<object?> { item }));
            else
    ...
    private static RuntimeState SnapshotState(RuntimeState state)
    {
        ...
        foreach ((string name, object? value) in state.Variables)
            snapshot.SetVariable(name, value);
        return snapshot;
    }
    private static async ValueTask<List<object?>> MaterializeSequenceAsync(object? source, string operation, CancellationToken ct)
    {
        ...
        if (source is not null && AsyncSequenceAdapter.CanEnumerate(source))
            return await AsyncSequenceAdapter.ToListAsync(source, ct).ConfigureAwait(false);
    ...
    private object? EvaluateUnaryExpression(BoundUnaryExpression unary, RuntimeState state, object? item)
    {
        ...
        if (unary.Descriptor.Evaluation == OperatorEvaluationKind.Custom)
            return EvaluateCustom(unary.Descriptor, operand);
    ...
    private object EvaluateBinaryExpression(BoundBinaryExpression binary, RuntimeState state, object? item)
    {
        ...
        if (binary.Descriptor.Evaluation == OperatorEvaluationKind.Custom)
            return EvaluateCustom(binary.Descriptor, leftValue, rightValue)!;
    ...
    {
        object? value = EvaluateExpression(between.Operand, state, item);
        object? lower = EvaluateExpression(between.Lower, state, item);
        object? upper = EvaluateExpression(between.Upper, state, item);
        if (between.Descriptor.Evaluation == OperatorEvaluationKind.Custom)
            return EvaluateCustom(between.Descriptor, value, lower, upper)!;
        if (between.Descriptor.Evaluation != OperatorEvaluationKind.Between)
            throw new InvalidOperationException($"Operator '{between.Operator}' is not a BETWEEN evaluator.");
    ...
    {
        if (_operatorEvaluators.TryEvaluate(descriptor, operands, _services, out object? result))
            return result;
    ...
    private bool EvaluatePredicate(BoundPredicateExpression predicate, RuntimeState state, object? item)
    {
        ...
        foreach (string capability in predicate.Descriptor.CapabilitiesFor(predicate.Operand.Type))
            if (!IsCapabilityAllowed(capability, value))
                throw new UnauthorizedAccessException($"Capability '{capability}' is required by predicate '{predicate.Predicate}'.");
    ...
    private static bool Contains(object? container, object? value)
    {
        ...
        if (container is IDictionary dictionary)
            return value is not null && dictionary.Contains(value);
        if (container is IEnumerable enumerable)
            return enumerable.Cast<object?>().Any(item => EqualsNormalized(item, value));
        return false;
    }
    ...
    {
        if (projection.Kind == OutputProjectionKind.WholeResult)
            return result;
        if (result is null)
            return null;
        ...
        if (result is System.Runtime.CompilerServices.ITuple tuple && index >= 0 && index < tuple.Length)
            return tuple[index];
        if (result is IList list && index >= 0 && index < list.Count)
            return list[index];
    ...
    {
        foreach (BoundStatement statement in script.Statements)
            foreach (string capability in CollectCapabilities(statement))
    ...
    private static IEnumerable<string> CollectCapabilities(BoundStatement statement)
    {
        switch (statement)
        {
            case BoundPipeline pipeline:
                foreach (BoundStage stage in pipeline.Stages)
                {
                    if (stage is BoundSentence sentence)
                        foreach (string capability in sentence.Implementation.Capabilities)
                            yield return capability;
                    if (stage is BoundScriptCall scriptCall)
                        foreach (BoundStatement child in scriptCall.Callable.Body.Statements)
                            foreach (string capability in CollectCapabilities(child))
                                yield return capability;
                    foreach (string capability in CollectExpressionCapabilities(stage))
                        yield return capability;
                }
                break;
            case BoundIf conditional:
                foreach (string capability in CollectExpressionCapabilities(conditional.Condition))
                    yield return capability;
                foreach (BoundStatement child in conditional.Then.Statements)
                    foreach (string capability in CollectCapabilities(child))
                        yield return capability;
                if (conditional.Else is not null)
                    foreach (BoundStatement child in conditional.Else.Statements)
                        foreach (string capability in CollectCapabilities(child))
                            yield return capability;
                break;
            case BoundForEach loop:
                foreach (BoundStatement child in loop.Body.Statements)
                    foreach (string capability in CollectCapabilities(child))
                        yield return capability;
                break;
            case BoundTry @try:
                foreach (BoundStatement child in @try.Body.Statements)
                    foreach (string capability in CollectCapabilities(child))
                        yield return capability;
                if (@try.Failure is not null)
                    foreach (BoundStatement child in @try.Failure.Statements)
                        foreach (string capability in CollectCapabilities(child))
                            yield return capability;
                if (@try.Finally is not null)
                    foreach (BoundStatement child in @try.Finally.Statements)
                        foreach (string capability in CollectCapabilities(child))
                            yield return capability;
                break;
        }
    ...
    {
        if (stage is BoundFilter filter)
            return CollectExpressionCapabilities(filter.Predicate);
        if (stage is BoundCheck check)
            return CollectExpressionCapabilities(check.Condition);
    ...
    private static IEnumerable<string> CollectExpressionCapabilities(BoundExpression expression)
    {
        ...
        switch (expression)
        {
            case BoundUnaryExpression unary:
                foreach (string capability in CollectExpressionCapabilities(unary.Operand))
                    yield return capability;
                break;
            case BoundBinaryExpression binary:
                foreach (string capability in CollectExpressionCapabilities(binary.Left))
                    yield return capability;
                foreach (string capability in CollectExpressionCapabilities(binary.Right))
                    yield return capability;
                break;
            case BoundBetweenExpression between:
                foreach (string capability in CollectExpressionCapabilities(between.Operand))
                    yield return capability;
                foreach (string capability in CollectExpressionCapabilities(between.Lower))
                    yield return capability;
                foreach (string capability in CollectExpressionCapabilities(between.Upper))
                    yield return capability;
                break;
            case BoundPredicateExpression boundPredicate:
                foreach (string capability in CollectExpressionCapabilities(boundPredicate.Operand))
                    yield return capability;
                break;
        }
    }
