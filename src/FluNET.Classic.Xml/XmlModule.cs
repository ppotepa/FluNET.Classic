...
public sealed class XmlModule : LanguageModule { public override string Name => "xml"; }

[Verb("PARSE"), Qualifier("XML"), ExecutionTrait(ExecutionTrait.Pure)]
...
[Verb("FORMAT"), Qualifier("XML"), ExecutionTrait(ExecutionTrait.Pure)]
...
public sealed class GetXmlElementText : IVerb<string?>, IGet, IFrom<XDocument>, IAt<XmlPath>, IPipelineProducer<string?>
{
    ...
    public GetXmlElementText([From] XDocument document, [At] XmlPath path) { _document = document; _path = path; }
    public ValueTask<string?> ExecuteAsync(VerbExecutionContext context, CancellationToken cancellationToken = default)
    {
        XElement? current = _document.Root; foreach (string segment in _path.Value.Split('/', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)) current = current?.Element(segment); return ValueTask.FromResult(current?.Value);
    }
}
