...
public sealed class ModuleQualityAnalyzer
{
    public IReadOnlyList<ModuleQualityIssue> Analyze(LanguageSnapshot snapshot)
    {
        ...
        foreach (VerbImplementationDescriptor implementation in snapshot.Verbs.SelectMany(x => x.Implementations))
        {
            ...
            if (sideEffect && implementation.Capabilities.Count == 0)
                issues.Add(new(LanguageDiagnosticSeverity.Warning, "FLU-SDK-Q001", $"Side-effecting implementation '{implementation.ImplementationType.FullName}' declares no capability.", implementation.StableId));
            if (implementation.Traits.Contains(ExecutionTrait.Retryable) && sideEffect && !implementation.Traits.Contains(ExecutionTrait.Idempotent))
                issues.Add(new(LanguageDiagnosticSeverity.Warning, "FLU-SDK-Q002", $"Retryable side-effecting implementation '{implementation.ImplementationType.FullName}' is not marked Idempotent.", implementation.StableId));
            if (pure && sideEffect)
                issues.Add(new(LanguageDiagnosticSeverity.Error, "FLU-SDK-Q003", $"Implementation '{implementation.ImplementationType.FullName}' cannot be both Pure and SideEffecting.", implementation.StableId));
            if (pure && implementation.Traits.Contains(ExecutionTrait.NonDeterministic))
                issues.Add(new(LanguageDiagnosticSeverity.Warning, "FLU-SDK-Q004", $"Pure implementation '{implementation.ImplementationType.FullName}' is also NonDeterministic.", implementation.StableId));
            if (transactional && !sideEffect)
                issues.Add(new(LanguageDiagnosticSeverity.Error, "FLU-SDK-Q007", $"Transactional implementation '{implementation.ImplementationType.FullName}' must also be SideEffecting.", implementation.StableId));
            if (hasAsyncShape && !streaming)
                issues.Add(new(LanguageDiagnosticSeverity.Warning, "FLU-SDK-Q008", $"Implementation '{implementation.ImplementationType.FullName}' exposes IAsyncEnumerable<T> but is not marked Streaming.", implementation.StableId));
            if (streaming && !hasAsyncShape)
                issues.Add(new(LanguageDiagnosticSeverity.Warning, "FLU-SDK-Q009", $"Implementation '{implementation.ImplementationType.FullName}' is marked Streaming but exposes no async-stream result or role.", implementation.StableId));
