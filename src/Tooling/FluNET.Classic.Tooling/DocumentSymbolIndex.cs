...
internal sealed class DocumentSymbolIndex
{
    ...
    private readonly IReadOnlyList<DocumentSymbolInfo> _declarations;

    private DocumentSymbolIndex(IReadOnlyList<IndexedOccurrence> occurrences, IReadOnlyList<DocumentSymbolInfo> declarations)
    {
    ...
    public static DocumentSymbolIndex Build(string source, ClassicLexer lexer, ClassicParser parser)
    {
        ...
        var declarations = parse.Success
            ? parse.Script.Statements.Select(statement => statement switch
            {
                DefinitionNode definition => new DocumentSymbolInfo(definition.Name, definition.Kind == DefinitionKind.Function ? "function" : "task", definition.Span),
                RecordDefinitionNode record => new DocumentSymbolInfo(record.Name, "record", record.Span),
                _ => null
            }).Where(x => x is not null).Cast<DocumentSymbolInfo>().ToArray()
            : Array.Empty<DocumentSymbolInfo>();
        ...
        return new(occurrences, declarations);
    }

    public IReadOnlyList<DocumentSymbolInfo> Definitions => _declarations.Concat(_occurrences
        .Where(x => x.IsDefinition)
        .Select(x => new DocumentSymbolInfo(x.Name, x.Kind, x.Span))).ToArray();
