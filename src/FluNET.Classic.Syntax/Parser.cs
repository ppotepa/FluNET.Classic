...
public sealed class ClassicParser
{
    ...
    private IfNode ParseIf()
    {
        ...
        BlockNode then = ParseBody(true);
        BlockNode? otherwise = null;
        if (IsWord("ELSE"))
        {
            Advance();
            otherwise = ParseBody(false);
        }
    ...
    private ForEachNode ParseForEach()
    {
        ...
        BlockNode body = ParseBody(false);
    ...
    private BlockNode ParseBody(bool stopAtElse)
    {
        ...
        {
            int start = Advance().Span.Start;
            List<StatementNode> statements = ParseStatements(true);
            int end = Current.Kind == TokenKind.RightBrace
                ? Advance().Span.End
                : statements.LastOrDefault()?.Span.End ?? start;
            return new(statements, TextSpan.FromBounds(start, end));
        }

        StatementNode? single = ParsePipeline(stopAtElse);
        return single is null
            ? new(Array.Empty<StatementNode>(), Current.Span)
            : new(new[] { single }, single.Span);
    }
    ...
    private FilterStageNode ParseFilter()
    {
        ...
        string? alias = ParseOptionalResultAlias(true);
    ...
    private CollectionStageNode ParseIntrinsic(IntrinsicDescriptor intrinsic)
    {
        ...
        string? alias = ParseOptionalResultAlias(true);
        int end = alias is not null ? Previous.Span.End : argument?.Span.End ?? source?.Span.End ?? operation.Span.End;
    ...
    private SentenceNode? ParseSentence()
    {
        ...
        while (!IsStageBoundary(Current))
        {
            ...
            {
                Flush();
                alias = ParseOptionalResultAlias(false);
                break;
            }
            if (IsWord("AS") && Peek(1).Kind == TokenKind.Variable)
            {
                Flush();
                alias = ParseOptionalResultAlias(true);
                break;
            }
    ...
    private ExpressionNode ParseBinary(int parentPrecedence, string[] stopWords)
    {
        ...
        while (!AtExpressionBoundary(stopWords))
        {
            ...
            if (descriptor.Arity == OperatorArity.Ternary)
            {
                ...
                left = new BinaryExpression(lower, "AND", upper, TextSpan.FromBounds(left.Span.Start, high.Span.End));
                continue;
            }
    ...
    private bool TryParsePredicate(ref ExpressionNode left)
    {
        ...
        ExpressionNode predicateExpression = new PredicateExpression(
            statePredicate.Name.ToUpperInvariant(),
            left,
    ...
    {
        string[] parts = (token.Value?.ToString() ?? string.Empty)
    ...
    {
        if (TryPeekOperator(OperatorArity.Ternary, out descriptor, out tokenCount)) return true;
    ...
    {
        foreach (OperatorDescriptor candidate in _language.Operators
                     .Where(x => x.Arity == arity)
                     .OrderByDescending(MaxSurfaceWordCount)
