...
{
    private static readonly HashSet<string> NamedPredicates = new(StringComparer.OrdinalIgnoreCase) { "OK", "VALID", "EMPTY" };
    private static readonly HashSet<string> CollectionOperations = new(StringComparer.OrdinalIgnoreCase) { "SORT", "GROUP", "TAKE", "SKIP", "DISTINCT", "COUNT" };
    ...
    {
        source ??= string.Empty; _tokens = _lexer.Lex(source); _position = 0; _diagnostics.Clear();
    ...
    {
        int start = Advance().Span.Start; ExpressionNode condition = ParseExpressionUntil("THEN"); ConsumeThen("FLU-SYN-101", "IF requires THEN.");
        BlockNode then = ParseBody(true); BlockNode? otherwise = null;
    ...
    private bool TryConsumePipelineContinuation()
    {
        ...
        if (IsWord("AND") && IsWord("THEN", 1)) { Advance(); Advance(); SkipNewLines(); return true; }
    ...
    private PipelineStageNode? ParseStage()
    {
        ...
        if (Current.Kind == TokenKind.Word && CollectionOperations.Contains(Current.Text)) return ParseCollection();
    ...
    private CollectionStageNode ParseCollection()
    {
        SyntaxToken operation = Advance(); int start = operation.Span.Start; string name = operation.Text.ToUpperInvariant(); ExpressionNode? source = null; ExpressionNode? argument = null;
        switch (name)
        {
            case "SORT":
            case "GROUP":
                if (!IsWord("BY")) source = ParseAtomic();
                ExpectWord("BY", "FLU-SYN-160", $"{name} requires BY."); argument = ParseAtomic();
                break;
            case "TAKE":
            case "SKIP":
                argument = ParseAtomic(); if (IsWord("FROM")) { Advance(); source = ParseAtomic(); }
                break;
            case "DISTINCT":
                if (!IsWord("BY") && !IsResultBinding() && !IsStageBoundary(Current)) source = ParseAtomic();
                if (IsWord("BY")) { Advance(); argument = ParseAtomic(); }
                break;
            case "COUNT":
                if (!IsResultBinding() && !IsStageBoundary(Current)) source = ParseAtomic();
                break;
        }
    ...
    private SentenceNode? ParseSentence()
    {
        ...
        if (Current.Kind == TokenKind.Word && _language.TryGetQualifier(Current.Text, out QualifierDescriptor qualifierDescriptor)) { qualifier = qualifierDescriptor.Name; Advance(); }
    ...
    {
        ExpressionNode left; int unary = UnaryPrecedence(Current);
        if (unary > 0) { SyntaxToken op = Advance(); ExpressionNode operand = ParseBinary(unary, stopWords); left = new UnaryExpression(op.Text.ToUpperInvariant(), operand, TextSpan.FromBounds(op.Span.Start, operand.Span.End)); }
        else left = ParsePrimary(stopWords);

        while (!AtExpressionBoundary(stopWords))
        {
            if (TryParsePredicate(ref left)) continue;
            int precedence = BinaryPrecedence(); if (precedence == 0 || precedence <= parentPrecedence) break;
            string operatorText = ConsumeBinaryOperator();
            if (operatorText == "BETWEEN")
            {
                ExpressionNode low = ParseBinary(precedence, stopWords);
                if (!IsWord("AND")) { _diagnostics.Add(new("FLU-SYN-170", "BETWEEN requires AND.", Current.Span)); break; }
                Advance(); ExpressionNode high = ParseBinary(precedence, stopWords);
                var lower = new BinaryExpression(left, ">=", low, TextSpan.FromBounds(left.Span.Start, low.Span.End));
                var upper = new BinaryExpression(left, "<=", high, TextSpan.FromBounds(left.Span.Start, high.Span.End));
                left = new BinaryExpression(lower, "AND", upper, TextSpan.FromBounds(left.Span.Start, high.Span.End)); continue;
            }
            ExpressionNode right = ParseBinary(precedence, stopWords); left = new BinaryExpression(left, operatorText, right, TextSpan.FromBounds(left.Span.Start, right.Span.End));
        }
        return left;
    }
    ...
    {
        if (IsWord("EXISTS")) { SyntaxToken predicate = Advance(); left = new PredicateExpression("EXISTS", left, TextSpan.FromBounds(left.Span.Start, predicate.Span.End)); return true; }
        ...
        Advance(); if (negate) Advance(); SyntaxToken predicateToken = Advance(); ExpressionNode predicateExpression = new PredicateExpression(predicateToken.Text.ToUpperInvariant(), left, TextSpan.FromBounds(left.Span.Start, predicateToken.Span.End));
        left = negate ? new UnaryExpression("NOT", predicateExpression, predicateExpression.Span) : predicateExpression; return true;
    }

    private string ConsumeBinaryOperator()
    {
        if (Current.Kind == TokenKind.Operator) return Advance().Text.ToUpperInvariant();
        string first = Advance().Text.ToUpperInvariant();
        if (first == "IS" && IsWord("NOT")) { Advance(); return "IS NOT"; }
        if ((first == "STARTS" || first == "ENDS") && IsWord("WITH")) { Advance(); return first + " WITH"; }
        return first;
    }
    ...
    {
        if (Current.Kind == TokenKind.Operator) return 4; if (Current.Kind != TokenKind.Word) return 0;
        string word = Current.Text.ToUpperInvariant();
        if (word == "OR") return 1; if (word == "AND") return 2; if (word == "IS") return 4;
        if (word is "CONTAINS" or "MATCHES" or "IN" or "BEFORE" or "AFTER" or "BETWEEN") return 3;
        if ((word == "STARTS" || word == "ENDS") && IsWord("WITH", 1)) return 3;
        return 0;
    }
